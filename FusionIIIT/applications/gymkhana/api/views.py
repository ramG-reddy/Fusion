...
class VoteIncrementAPIView(APIView):
    def post(self, request):
        ...
        for choice_data in data:
            ...
            try:
                choice_instance = Voting_choices.objects.get(
                    poll_event_id=poll_event_id, title=title
                )
        ...
        return Response(
            {"message": "Votes incremented successfully"}, status=status.HTTP_200_OK
        )


class VotingPollsDeleteAPIView(APIView):
    def post(self, request):
        Voting_poll_id = request.data.get(
            "id"
        ...
        try:
            ...
            return Response(
                {"error": "Voting Poll not found."}, status=status.HTTP_404_NOT_FOUND
            )
...
class ClubMemberApproveView(generics.UpdateAPIView):
    def post(self, request):
        ...
        club_member.status = (
            "confirmed"  # Assuming 'confirmed' is the status for approval
        )
        club_member.save()
...
class AddMemberToClub(APIView):
    def post(self, request):
        ...
        if serializer.is_valid():
            club_id = request.data.get(
                "club"
            )  # Assuming 'club_id' is passed in the request data
            try:
...
class CreateVotingPollAPIView(APIView):
    def post(self, request):
        ...
        if voting_poll_serializer.is_valid():
            ...
            voting_choices_serializer = Voting_choicesSerializer(
                data=choices_data, many=True
            )
            if voting_choices_serializer.is_valid():
                ...
                return Response(
                    {"voting_choices_errors": voting_choices_serializer.errors},
                    status=status.HTTP_400_BAD_REQUEST,
                )
        else:
            return Response(
                {"voting_poll_errors": voting_poll_serializer.errors},
                status=status.HTTP_400_BAD_REQUEST,
            )
