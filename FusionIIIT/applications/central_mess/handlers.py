...
def add_vacation_food_request(request, student):
    ...
    vacation_check = Vacation_food.objects.filter(student_id=student).prefetch_related(
        "student_id",
        "student_id__id",
...
def handle_menu_change_response(request):
    ...
    if stat == "2":
        ...
        obj = Menu.objects.get(
            Q(meal_time=application.dish.meal_time)
            & Q(mess_option=application.dish.mess_option)
        )
...
def add_leave_request(request, student):
    ...
    rebate_check = Rebate.objects.select_related(
        "student_id",
        "student_id__id",
        "student_id__id__user",
        "student_id__id__department",
    ).filter(student_id=student, status__in=["1", "2"])
...
def rebateCountFn(start_date, end_date, student_id):
    ...
    try:
        existing_student = Monthly_bill.objects.get(
            student_id=student_id, month=start_date_month, year=start_date_year
        )
        ...
        if rebate_count_days_next_month != 0:
            new_student = Monthly_bill.objects.create(
                student_id=student_id,
                month=end_date_month,
                year=start_date_year,
                rebate_count=rebate_count_days_next_month,
            )
            new_student.save()
    except:
        new_student = Monthly_bill.objects.create(
            ...
            year=start_date_year,
            rebate_count=rebate_count_days,
        )
        new_student.save()
        if rebate_count_days_next_month != 0:
            new_student = Monthly_bill.objects.create(
...
def add_mess_committee(request, roll_number):
    ...
    try:
        ...
        check_obj = HoldsDesignation.objects.select_related().filter(
            Q(user__username=studentHere)
            & (
                Q(designation__name__contains="mess_committee")
                | Q(designation__name__contains="mess_convener")
            )
    ...
    for student in students:
        ...
        if start_date_month == month_previous:
            if end_date_month == month_previous:
                no_of_days = (
                    no_of_days + abs((student.end_date - student.start_date).days) + 1
                )
            elif end_date_month == today_g.month:
                no_of_days = (
                    no_of_days
                    + abs((last_day_prev_month - student.start_date).days)
                    + 1
                )
        elif end_date_month == month_previous:
            no_of_days = (
                no_of_days + abs((student.end_date - student.start_date).days) + 1
            )
        elif end_date_month == today_g.month:
            no_of_days = (
                no_of_days + abs((last_day_prev_month - first_day_prev_month).days) + 1
            )
    print(no_of_days)
...
def generate_bill():
    ...
    for student in student_all:
        ...
        try:
            monthly_bill_obj = Monthly_bill.objects.get(
                student_id=student_id, month=previous_month, year=previous_month_year
            )
            ...
            new_monthly_bill_obj = Monthly_bill(
                student_id=student_id,
    ...
    if status == "accept":
        ...
        try:
            ...
            new_reg = Reg_main(
                student_id=student,
                program=program,
                current_mess_status=mess_status,
                balance=amount,
                mess_option=mess,
            )
            new_reg.save()
        new_reg_record = Reg_records(
            student_id=student, start_date=start_date, end_date=sem_end_date
        )
        new_reg_record.save()

        new_payment_record = Payments(
            student_id=student,
            amount_paid=amount,
    ...
    if status == "accept":
        try:
            ...
            if end_date == date.today():
                reg_main.current_mess_status = "Deregistered"
            reg_record_obj = Reg_records.objects.filter(student_id=student).latest(
                "start_date"
            )
...
def update_month_bill(request):
    ...
    fixed_amount_per_month = int(bill_base_amount) * 30

    reg_main_obj = Reg_main.objects.get(student_id=student)
    curr_balance = reg_main_obj.balance
    try:
        existing_monthly_bill_object = Monthly_bill.objects.get(
            student_id=studentHere, month=month, year=year
        )
        ...
        new_monthly_bill_obj = Monthly_bill(
            student_id=studentHere,
            month=month,
            year=year,
            total_bill=new_amount,
            amount=fixed_amount_per_month,
        )
...
def handle_add_reg(request):
    ...
    new_reg_record = Reg_records(
        student_id=student, start_date=start_date, end_date=latest_end_date
    )
    new_reg_record.save()
