...
def check_deregistration(student):
    try:
        reg_end_date = Reg_records.objects.filter(student_id=student).latest(
            "start_date"
        )
...
def check_daily(student, per_day_cost):
    two_day_amount = 2 * int(per_day_cost)
    ...
    if balance <= two_day_amount:
        ...
        reg_end_date_obj = Reg_records.objects.filter(student_id=student).latest(
            "start_date"
        )
...
def generate_per_day_bill():
    ...
    for student in registered_students:
        ...
        try:
            rebate_obj = Rebate.objects.get(
                student_id=student_id,
                start_date__lte=today_g,
                end_date__gte=today_g,
                status=2,
            )  # TODO check for the correct value of accepting status
            try:
                monthly_bill_object = Monthly_bill.objects.get(
                    student_id=student_id, month=this_month, year=this_year
                )
                monthly_bill_object.rebate_count = monthly_bill_object.rebate_count + 1
                monthly_bill_object.rebate_amount = (
                    monthly_bill_object.rebate_amount + per_day_cost
                )
                monthly_bill_object.save()
            except:
                new_monthly_bill_object = Monthly_bill(
                    student_id=student_id,
                    month=this_month,
                    year=this_year,
                    amount=amount,
            ...
            try:
                ...
                monthly_bill_object.total_bill = (
                    monthly_bill_object.total_bill + per_day_cost
                )
                print(monthly_bill_object.total_bill)
        ...
        if today_g == last_day_of_this_month and current_balance < amount:
            ...
            try:
                reg_record_end_date = Reg_records.objects.filter(
                    student_id=student_id
