...
from applications.examination.models import authentication, grade, hidden_grades
...
def fetch_roll_of_courses(request):
    ...
    if request.method == "POST":
        ...
        if course_id is None:
            return Response(
                {"error": "Course ID is required in the request parameters"},
                status=status.HTTP_400_BAD_REQUEST,
            )

        # Filter students by the provided course ID and working year
        obj = course_registration.objects.filter(
            course_id=course_id, working_year=working_year
        )
...
def add_student_details(request):
    ...
    if request.method == "POST":
        ...
        return Response(
            {"success": f"{success_count} student(s) added successfully"}, status=201
        )
...
def update_hidden_grade(request):
    ...
    if request.method == "GET":
        ...
        # Serialize the queryset
        students_serialized = serializers.CourseRegistrationSerializer(
            students, many=True
        ).data

        # Prepare the response data
        resp = {"students": students_serialized}
        ...
        for data in data_list:
            ...
            if not all([student_id, grade]):
                return JsonResponse(
                    {"error": "Incomplete data provided for one of the students"},
                    status=status.HTTP_400_BAD_REQUEST,
                )
            ...
            try:
                ...
                hidden_grade_obj = hidden_grades.objects.create(
                    course_id=course_id, student_id=student_id, grade=grade
                )

        return JsonResponse(
            {"message": "Hidden grades updated successfully"},
...
def check_all_authenticators(request):
    ...
    if request.method == "GET":
        ...
        auth_objects = authentication.objects.filter(
            year__year=year, course_id=course_id
        )

        if not auth_objects.exists():
            return Response(
                {"error": "No authentication entries found for the provided year."},
        ...
        for auth_object in auth_objects:
            if not (
                auth_object.authenticator_1
                and auth_object.authenticator_2
                and auth_object.authenticator_3
            ):
...
def update_authenticator(request):
    ...
    if request.method == "PATCH":
        ...
        if not auth_objects.exists():
            return Response(
                {
                    "error": "No authentication entries found for the provided year and course id."
                },
                status=status.HTTP_404_NOT_FOUND,
            )
        ...
        for auth_object in auth_objects:
            if authenticator_number == 1:
                ...
                return Response(
                    {"error": "Invalid authenticator number"},
                    status=status.HTTP_400_BAD_REQUEST,
                )
        ...
        return Response(
            {
                "message": f"Authenticator {authenticator_number} toggled successfully for the year {year} and course id {course_id}"
            },
...
def publish_grade(request):
    ...
    if auth_obj:
        if (
            auth_obj.authenticator_1
            and auth_obj.authenticator_2
            and auth_obj.authenticator_3
        ):
            ...
            return JsonResponse(
                {"error": "Not all authenticators are True"}, status=400
            )
    else:
...
def generate_transcript(request):
    ...
    if request.method == "POST":
        ...
        total_courses_registered = (
            Student_grades.objects.filter(roll_no=student_id, semester__lte=semester)
        ...
        return JsonResponse(
            {
                ...
                "total_courses_registered": total_courses_registered,
            }
        )
    else:
        return JsonResponse({"error": "Invalid request method"})
    ...
    if request.method == "POST":
        ...
        for course in courses:
            try:
                ...
                print(
                    f"Error creating authentication object for course ID {course['id']}: {e}"
                )
        ...
        serialized_data = [
            {
                "id": obj.id,
                "authenticator_1": obj.authenticator_1,
                "authenticator_2": obj.authenticator_2,
                "authenticator_3": obj.authenticator_3,
                "year": obj.year.year,
                "course_id": obj.course_id_id,
            }
            for obj in created_authentications
        ]
        ...
        try:
            ...
            for student_data in updated_students_data:
                ...
                if not (
                    ...
                    return JsonResponse(
                        {"error": "Incomplete data provided"}, status=400
                    )
            ...
            return JsonResponse(
                {"message": "Student grades updated successfully"}, status=200
            )
            ...
            return JsonResponse(
                {"error": "Missing required field: " + str(e)}, status=400
            )
...
def get_registered_students_roll_no(request):
    ...
    try:
        ...
        registrations = course_registration.objects.filter(
            course_id=course_id, working_year=year
        )
        ...
        for registration in registrations:
            ...
            try:
                ...
                student_grade = Student_grades.objects.get(
                    roll_no=student_data["roll_no"], course_id=course_id
                )
                ...
                print(
                    "Didn't find grades for roll_no:",
