...
from applications.academic_information.models import Calendar, Curriculum_Instructor
...
from applications.programme_curriculum.models import CourseSlot, Semester
...
def academic_procedures_student(request):
    ...
    if str(des.designation) == "student":
        ...
        pre_registration_date_flag = get_pre_registration_eligibility(
            current_date, user_sem, acad_year
        )
        ...
        for course_slot in next_sem_registration_courses:
            ...
            next_sem_courses.append(
                {
                    "slot_id": course_slot.id,
                    "slot_name": course_slot.name,
                    "slot_type": course_slot.type,
                    "semester": course_slot.semester.semester_no,
                    "slot_info": course_slot.course_slot_info,
                    "courses": courselist,
                }
            )
        ...
        try:
            ...
            for pre_registered_course in pre_registered_courses:
                ...
                if (
                    ...
                    pre_registered_course_show.append(
                        {
                            "slot_name": pre_registered_course.course_slot_id.name,
        ...
        for course_option in add_courses_options:
            ...
            add_courses_options_show.append(
                {
                    "slot_id": course_slot.id,
                    "slot_name": course_slot.name,
                    "slot_type": course_slot.type,
        ...
        for course in drop_courses_options:
            drop_courses_options_show.append(
                {
                    "course_id": course.id,
                    "name": course.name,
                    "credit": course.credit,
        ...
        replace_courses_options = get_replace_course_options(
            currently_registered_course, batch.year
        )
        print("replace", replace_courses_options)

        backlogCourseList = []
        auto_backlog_courses = list(
            SemesterMarks.objects.filter(student_id=obj, grade="F")
        )
        auto_backlog_courses_list = []
        for i in auto_backlog_courses:
            if not i.curr_id.courseslots.filter(type__contains="Optional").exists():
                auto_backlog_courses_list.append(
                    {
                        "course_name": i.curr_id.name,
                        "course_code": i.curr_id.code,
                        "course_version": i.curr_id.version,
                        "course_credit": i.curr_id.credit,
                        "course_grade": i.grade,
                    }
                )

        backlogCourses = backlog_course.objects.select_related(
            "course_id", "student_id", "semester_id"
        ).filter(student_id=obj)
        for i in backlogCourses:
            summer_course = "Yes" if i.is_summer_course else "No"
            course_details = (
                i.course_id.course_details if i.course_id.course_details else "N/A"
            )

            backlogCourseList.append(
                [
                    i.course_id.course_name,
                    course_details,
                    i.semester_id.semester_no,
                    summer_course,
                ]
            )
        ...
        faculty_list = serializers.HoldsDesignationSerializer(
            get_faculty_list(), many=True
        ).data
...
def gen_roll_list(request):
    ...
    return JsonResponse(
        {"students": students, "batch": batch, "course": course_id}, status=200
    )
...
def add_course(request):
    try:
        ...
        for i in range(1, count + 1):
            ...
            try:
                ...
                if course_registration.objects.filter(
                    working_year=current_user.batch_id.year,
                    course_id=course_id_instance,
                ).count() < courseslot_id_instance.max_registration_limit and (
                    course_registration.objects.filter(
                        course_id=course_id_instance, student_id=current_user
                    ).count()
                    == 0
                ):
        ...
        course_registration_data = serializers.CourseRegistrationSerializer(
            course_registration_data, many=True
        ).data
        res = {
            "message": "Courses successfully added",
            "courses_added": course_registration_data,
        }
...
def get_user_info(request):
    ...
    details = {"user_serializer_Data": details1, "ExtraInfoSerializer_Data": details2}
...
def student_view_registration(request):
    try:
        ...
        obj = StudentRegistrationChecks.objects.filter(
            semester_id_id=sem_id, student_id=student_id
        )

        # serialize the data for displaying
        serializer = serializers.StudentRegistrationChecksSerializer(
            obj, many=True
        ).data
...
def student_pre_registration(request):
    try:
        ...
        current_user_instance = (
            Student.objects.all().filter(id=current_user["id"]).first()
        )
        ...
        # filter based on the semester id and student id
        obj = StudentRegistrationChecks.objects.filter(
            semester_id_id=sem_id, student_id=s_id
        )
        # serialize the data for displaying
        student_registration_check = serializers.StudentRegistrationChecksSerializer(
            obj, many=True
        ).data
        ...
        try:
            ...
            if (
                ...
                and student_registration_check[0]["pre_registration_flag"]
            ):
                return Response(
                    data={"message": "You have already registered for this semester"},
        ...
        try:
            serialized_reg_curr = serializers.InitialRegistrationSerializer(
                reg_curr, many=True
            ).data
            ...
            return Response(
                data={
                    "message": "Successfully Registered for the courses.",
                    "registrations": serialized_reg_curr,
                },
                status=status.HTTP_200_OK,
            )
        except Exception as e:
            return Response(
                data={"message": "Error in Registration.", "error": str(e)},
...
def student_final_registration(request):
    try:
        ...
        sem_id_instance = Semester.objects.get(id=request.data.get("semester"))
        sem_id = serializers.SemesterSerializer(sem_id_instance).data["id"]
        registration_status = StudentRegistrationChecks.objects.filter(
            student_id=current_user["id"], semester_id=sem_id
        )
        registration_status = serializers.StudentRegistrationChecksSerializer(
            registration_status, many=True
        ).data

        if (
            len(registration_status) > 0
            and registration_status[0]["pre_registration_flag"] == False
        ):
            return Response(
                data={"message": "Student haven't done pre registration yet."},
                status=status.HTTP_400_BAD_REQUEST,
            )
        mode = str(request.data.get("mode"))
        transaction_id = str(request.data.get("transaction_id"))
        ...
        try:
            registration_status = StudentRegistrationChecks.objects.filter(
                student_id=current_user_instance, semester_id=sem_id
            ).update(final_registration_flag=True)
            return Response(
                data={"message": "Final Registration Successfull"},
                status=status.HTTP_200_OK,
            )
        except Exception as e:
            return Response(
                data={"message": "Final Registration Failed ", "error": str(e)},
                status=status.HTTP_500_INTERNAL_SERVER_ERROR,
            )
    except Exception as e:
        return Response(
...
def student_backlog_courses(request):
    try:
        stu_id = Student.objects.select_related("id", "id__user", "id__department").get(
...
def acad_view_reigstrations(request):
    try:
        ...
        sem_id = serializers.SemesterSerializer(sem_id_instance).data["id"]
        obj = StudentRegistrationChecks.objects.filter(
            semester_id_id=sem_id, final_registration_flag=True
        )
        student_registration_check = serializers.StudentRegistrationChecksSerializer(
            obj, many=True
        ).data
...
def configure_pre_registration_date(request):
    try:
        ...
        return Response(
            data={
                "message": "Pre registration for semester "
                + str(semester)
                + " will be opened from "
                + str(from_date)
                + " to "
...
def configure_final_registration_date(request):
    try:
        ...
        return Response(
            data={
                "message": "Physical Reporting at the Institute will be opened from "
                + str(from_date)
                + " to "
                + str(to_date)
                + ". ",
...
def add_course_to_slot(request):
    ...
    try:
        ...
        return JsonResponse(
            {
                "message": f"Course {course_code} added to slot {course_slot_name} successfully."
            },
            status=200,
        )
...
def remove_course_from_slot(request):
    ...
    try:
        ...
        return JsonResponse(
            {
                "message": f"Course {course_code} removed from slot {course_slot_name} successfully."
            },
            status=200,
        )
    except CourseSlot.DoesNotExist:
...
def faculty_assigned_courses(request):
    try:
        current_user = request.user
        curriculum_ids = Curriculum_Instructor.objects.filter(
        ...
        for curriculum_id in curriculum_ids:
            course_info = (
                Curriculum.objects.filter(curriculum_id=curriculum_id)
                .values_list(
                    "course_code",
                    "course_type",
                    "programme",
...
def get_next_sem_courses(request):
    try:
        ...
        batch_id = Student.objects.filter(
            programme=programme, batch=batch, specialization=branch
        )[0].batch_id
...
def add_one_course(request):
    try:
        ...
        try:
            ...
            if (
                course_registration.objects.filter(
                    course_slot_id_id=courseslot_id, student_id=current_user
                ).count()
                == 1
                and courseslot_id.type != "Swayam"
            ):
                already_registered_course_id = course_registration.objects.filter(
                    course_slot_id_id=courseslot_id, student_id=current_user
                )[0].course_id
                print(already_registered_course_id)
                msg = (
                    "Already Registered in the course : "
                    + already_registered_course_id.code
                    + "-"
                    + already_registered_course_id.name
                )
            ...
            if (
                ...
                return JsonResponse(
                    {"message": "Already registered in this course!"}, status=200
                )
            ...
            if course_registration.objects.filter(
                ...
                p.save()
                return JsonResponse(
                    {"message": "Course added successfully"}, status=200
                )
            else:
                return JsonResponse(
                    {"message": "Course not added because seats are full!"}, status=404
                )
        except Exception as e:
            print(e)
...
def verify_course(request):
    ...
    curr_id = batch.curriculum
    curr_sem_id = Semester.objects.get(
        curriculum=curr_id, semester_no=obj2.curr_semester_no
    )
    ...
    return JsonResponse(
        {
            "details": details,
            "course_list": serializers.CourseSerializer(course_list, many=True).data,
            "semester_list": serializers.SemesterSerializer(
                semester_list, many=True
            ).data,
            "date": date,
        }
    )
