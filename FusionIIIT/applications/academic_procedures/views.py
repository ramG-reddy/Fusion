...
from django.db.models import CharField, F, Max, Sum, Value
...
def academic_procedures(request):
    ...
    if request.session.get("currentDesignationSelected") == "student":
        ...
        request.session.get("currentDesignationSelected") == "faculty"
        or request.session.get("currentDesignationSelected") == "Associate Professor"
        or request.session.get("currentDesignationSelected") == "Professor"
        or request.session.get("currentDesignationSelected") == "Assistant Professor"
    ):
...
def academic_procedures_student(request):
    ...
    if str(des.designation) == "student":
        ...
        auto_backlog_courses = list(
            Student_grades.objects.filter(roll_no=obj, grade="F")
        )
...
def gen_course_list(request):
    if request.POST:
        try:
            ...
            obj = (
                course_registration.objects.filter(
                    course_id__id=course_id, working_year=batch
                )
                ...
                verified_students.append(
                    {
                        ...
                        "name": registration.student_id.id.user.first_name
                        + " "
                        + registration.student_id.id.user.last_name,
                        "department": registration.student_id.id.department.name,
                    }
                )
        html = render_to_string(
            "academic_procedures/gen_course_list.html",
            {"students": verified_students, "batch": batch, "course": course_id},
            request,
        )
        maindict = {"html": html}
    ...
    if request.method == "POST":
        try:
            ...
            try:
                ...
                if (
                    student_registeration_check
                    and student_registeration_check.pre_registration_flag == True
                ):
                    messages.error(
                        request, "You have already registered for next semester"
                    )
            ...
            for course_slot in course_slots:
                ...
                for course_priority in course_priorities:
                    ...
                    course_id_for_model = Courses.objects.get(id=course_id)
                    current_combination = (
                        course_slot_id_for_model.id,
                        course_id_for_model.id,
                    )
            ...
            for i in range(1, sheet.nrows):
                ...
                try:
                    ...
                    if roll_no not in currroll:
                        student_check = StudentRegistrationChecks(
                            student_id=student,
                            semester_id=sem_id,
                            pre_registration_flag=True,
                            final_registration_flag=False,
                    ...
                    print("----------------------", e)
                pre_registration = InitialRegistration(
                    student_id=student,
        ...
        try:
            ...
            for i in range(1, count + 1):
                ...
                try:
                    ...
                    registered_course = course_registration.objects.filter(
                        student_id=current_user, course_slot_id=courseslot_id
                    ).first()
        ...
        if student_obj:
            reg_table = (
                ...
                .filter(
                    ...
                    student_id__finalregistration__verified=False,
                    student_id__finalregistration__semester_id=student_obj[
                        0
                    ].semester_id,
                )
                .select_related(
        ...
        if student_obj:
            table += (
                ...
                .annotate(
                    ...
                    pwd_status=Value("YES", output_field=CharField())
        ...
        try:
            ...
            try:
                ...
                if course_registration.objects.filter(
                    ...
                    return JsonResponse(
                        {"message": "Course not added because seats are full!"},
                        status=200,
                    )
            except Exception:
    ...
    if request.method == "POST":
        try:
            ...
            if course_registration.objects.filter(
                ...
                return JsonResponse(
                    {"message": "Cannot Replace to this course seats are full!"},
                    status=200,
                )
...
def replaceSwayam(request):
    if request.POST:
        ...
        current_sem_courses = get_currently_registered_elective(roll_no, curr_sem_id)
        ...
        semester_list = Semester.objects.all()
        html = render_to_string(
            "academic_procedures/studentSwayam.html",
            {
                "details": details,
                "dict2": dict2,
                "course_list": course_list,
...
def get_currently_registered_elective(student_id, semester_id):
    ...
    for registration in registrations:
        if registration.course_slot_id.type == "Optional Elective":
            courses.append(
                (registration.course_slot_id, registration.course_id, registration.id)
            )
    return courses
...
def swayam_replace(request):
    if request.method == "POST":
        ...
        try:
            ...
            swayam_course_id_value_array = [
                int(id_str) for id_str in swayam_course_id_value.split(",")[:-1]
            ]
            ...
            swayam_course_slot_id_value_array = [
                int(slot_str)
                for slot_str in swayam_course_slot_id_value.split(",")[:-1]
            ]
            ...
            swayam_semester_id_value_array = [
                int(semester_str)
                for semester_str in swayam_semester_id_value.split(",")[:-1]
            ]
            ...
            # new_row_data = []
            for i in range(n):
                course_id_model = Courses.objects.get(
                    id=swayam_course_id_value_array[i]
                )
                # print(course_id_model)
                semester_id_model = Semester.objects.get(
                    id=swayam_semester_id_value_array[i]
                )
                student_id_model = Student.objects.get(id=user_value)
                course_slot_id_model = CourseSlot.objects.get(
                    id=swayam_course_slot_id_value_array[i]
                )
...
def register_backlog_course(request):
    if request.method == "POST":
        try:
            ...
            course_id = Courses.objects.get(
                code=request.POST.get("courseCode"), version=request.POST.get("Version")
            )
            course_slots = course_id.courseslots.all()
            course_slot_id = ""
            if course_slots:
                course_slot_id = CourseSlot.objects.filter(
                    id=course_slots[0].id
                ).first()
            ...
            try:
                if (
                    course_registration.objects.filter(
                        course_id=course_id, student_id=current_user, semester_id=sem_id
                    ).count()
                    == 0
                ):
                    p = course_registration(
                    ...
                    p.save()
                    return JsonResponse(
                        {"message": "Successfully Registered Backlog course"},
                        status=200,
                    )
                else:
                    return JsonResponse(
                        {"message": "Already Registered Backlog course"}, status=200
                    )
            except Exception as e:
                print(str(e))
                return JsonResponse(
                    {"message": "Error Registering course " + str(e)}, status=500
                )
        except Exception as e:
            print(str(e))
            return JsonResponse(
                {"message": "Adding Backlog Failed " + str(e)}, status=500
            )
