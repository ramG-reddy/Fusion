    ...
    for result in inbox_files:
        ...
        if request_object:
            element = [
                ...
                request_object.requestCreatedBy,
                file_obj.id,
            ]
            obj.append(element)
...
def view_file(request, id, url):
    ...
    current_user = (
        Tracking.objects.filter(file_id=file1).order_by("-receive_date")[0].receiver_id
    )
    print(current_user)
...
def handleEngineerProcessRequests(request):
    if request.method == "POST":
        ...
        Requests.objects.filter(id=request_id).update(
            engineerProcessed=1, status="Approved by the Engineer"
        )
        ...
        inbox_files = view_inbox(
            username=request.user, designation=desg, src_module="IWD"
        )
        ...
        for result in inbox_files:
            ...
            file_obj = File.objects.get(
                src_object_id=request_object.id, src_module="IWD"
            )
            if request_object:
                element = [
                    request_object.id,
...
def handleDeanProcessRequests(request):
    if request.method == "POST":
        ...
        Requests.objects.filter(id=request_id).update(
            deanProcessed=1, status="Approved by the dean"
        )
        desg = request.session.get("currentDesignationSelected")
...
def handleDirectorApprovalRequests(request):
    if request.method == "POST":
        ...
        if obj.get("action") == "approve":
            ...
            Requests.objects.filter(id=request_id).update(
                directorApproval=1, status="Approved by the director"
            )
        else:
            message = "Request_rejected"
            Requests.objects.filter(id=request_id).update(
                directorApproval=-1, status="Rejected by the director"
            )
...
def rejectedRequests(request):
    ...
    return render(
        request,
        "iwdModuleV2/rejectedRequests.html",
        {"obj": obj, "holdsDesignations": holdsDesignations},
    )
...
def updateRejectedRequests(request):
    ...
    return render(
        request,
        "iwdModuleV2/updateRequests.html",
        {"obj": obj, "holdsDesignations": holdsDesignations},
    )
...
def issueWorkOrder(request):
    ...
    for result in inbox_files:
        ...
        request_object = Requests.objects.filter(
            id=src_object_id, directorApproval=1, issuedWorkOrder=0
        ).first()
        if request_object:
            element = [
                request_object.id,
...
def workOrder(request):
    if request.method == "POST":
        ...
        Requests.objects.filter(id=request.POST["id"]).update(
            status="Work Order issued", issuedWorkOrder=1
        )
...
def workCompleted(request):
    if request.method == "POST":
        Requests.objects.filter(id=request.POST["id"]).update(
            workCompleted=1, status="Work Completed"
        )
        obj = []
        requestsObject = Requests.objects.filter(issuedWorkOrder=1, billGenerated=0)
        for x in requestsObject:
...
def generateFinalBill(request):
    if request.method == "POST":
        ...
        c.drawString(
            100,
            y_position - 80,
            f"Total Amount (in Rupees): {total_amount_to_be_paid:.2f}",
        )
...
def handleBillGeneratedRequests(request):
    if request.method == "POST":
        requestId = request.POST.get("id", 0)
        Requests.objects.filter(id=requestId).update(
            status="Bill Generated", billGenerated=1
        )
        obj = []
        requestsObject = Requests.objects.filter(issuedWorkOrder=1, billGenerated=0)
        for x in requestsObject:
...
def handleProcessedBills(request):
    if request.method == "POST":
        ...
        Requests.objects.filter(id=request_id).update(
            billProcessed=1, status="Final Bill Processed"
        )
...
def settleBillsView(request):
    ...
    for x in inbox_files:
        ...
        element = [
            bills_object.request_id.id,
            bills_object.file,
            bills_object.file.url,
            request_object.billSettled,
            file_obj.id,
            file_obj.id,
        ]
        obj.append(element)
...
def handleSettleBillRequests(request):
    if request.method == "POST":
        ...
        Requests.objects.filter(id=request_id).update(
            status="Final Bill Settled", billSettled=1
        )
...
def editBudget(request):
    if request.method == "POST":
        ...
        Budget.objects.filter(id=budgetId).update(
            name=budgetName, budgetIssued=budgetIssued
        )
