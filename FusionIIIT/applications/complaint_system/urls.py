...
urlpatterns = [
    ...
    url(
        r"^caretaker/(?P<complaint_id>[0-9]+)/feedback/$", views.submitfeedbackcaretaker
    ),
    # supervisor
    ...
    url(
        r"^supervisor/detail/(?P<detailcomp_id1>[0-9]+)/$",
        views.detail3,
        name="detail3",
    ),
    ...
    url(
        r"^supervisor/(?P<complaint_id>[0-9]+)/(?P<status>[0-9]+)/$",
        views.changestatussuper,
    ),
