    ...
    if extrainfo.user_type == "student":
        ...
        for reg in register:
            course = Courses.objects.select_related().get(
                pk=reg.course_id.id
            )  # Optimized retrieval using primary key
    ...
    if (
        ...
        for ins in instructor:
            if ins.course_id.code == course_code and ins.course_id.version == Decimal(
                ...
                registered_students_serialiser = StudentSerializer(
                    students_info, many=True
                )
        ...
        try:
            student_grades = Student_grades.objects.select_related().filter(
                course_id=course
            )
