...
def otheracademic(request):
    ...
    if request.user.extrainfo.user_type != "student":
        ...
        if user.extrainfo.student.programme == "B.Tech":
            ...
            user.extrainfo.student.programme == "M.Tech"
            or user.extrainfo.student.programme == "PhD"
        ):
...
def leave_form_submit(request):
    ...
    if request.method == "POST":
        ...
        otheracademic_notif(
            request.user, leave_hod, "ug_leave_hod", leave.id, "student", message
        )
        if leave:
...
def leaveApproveForm(request):
    ...
    if "HOD" in design:
        ...
        leave_ids = [
            msg["src_object_id"] for msg in inbox if msg["subject"] == "ug_leave"
        ]

        form_data = LeaveFormTable.objects.filter(id__in=leave_ids)
        return render(
            request, "otheracademic/leaveformreciever.html", {"form_data": form_data}
        )
    else:
...
def leaveStatus(request):
    ...
    return render(
        request,
        "otheracademic/leaveStatus.html",
        {"form_data": form_data, "roll_no": roll_no},
    )
...
def leaveStatus_Dip(request):
    ...
    form_data = LeaveFormTable.objects.filter(id__in=leave_ids)
    return render(
        request, "otheracademic/leaveStatus_Dip.html", {"form_data": form_data}
    )
...
def leavePgSubmit(request):
    ...
    if request.method == "POST":
        ...
        message = "A new leave application"
        otheracademic_notif(
            request.user, tasupervisor, "pg_leave_ta", leave.id, "student", message
        )
        if leave:
...
def leaveApproveTA(request):
    ...
    return render(
        request,
        "otheracademic/leaveApproveTA.html",
        {"form_data": form_data, "roll_no": roll_no},
    )
...
def approve_leave_ta(request, leave_id):
    ...
    otheracademic_notif(
        request.user, thesis_user, "pg_leave_thesis", leave_id, "student", message
    )

    return redirect(
        "/otheracademic/leaveApproveTA"
...
def reject_leave_ta(request, leave_id):
    ...
    return redirect(
        "/otheracademic/leaveApproveTA"
    )  # Redirect to appropriate page after rejection
...
def leaveApproveThesis(request):
    ...
    return render(
        request,
        "otheracademic/leaveApproveThesis.html",
        {"form_data": form_data, "roll_no": roll_no},
    )
...
def approve_leave_thesis(request, leave_id):
    ...
    otheracademic_notif(
        request.user, hod_user, "pg_leave_hod", leave_id, "student", message
    )

    return redirect(
        "/otheracademic/leaveApproveThesis"
    )  # Redirect to appropriate page after approval
...
def leaveApproveHOD(request):
    ...
    if "HOD" in design:
        ...
        leave_ids = [
            msg["src_object_id"] for msg in inbox if msg["subject"] == "pg_leave"
        ]
        form_data = LeavePG.objects.filter(id__in=leave_ids)
        roll_no = request.user.username
        return render(
            request,
            "otheracademic/leaveApproveHOD.html",
            {"form_data": form_data, "roll_no": roll_no},
        )
    else:
...
def leaveStatusPG(request):
    ...
    return render(
        request,
        "otheracademic/leaveStatusPG.html",
        {"form_data": form_data, "roll_no": roll_no},
    )
...
def leaveStatusPG_Dip(request):
    ...
    form_data = LeavePG.objects.filter(id__in=leave_ids)
    return render(
        request, "otheracademic/leaveStatusPG_Dip.html", {"form_data": form_data}
    )
...
def graduate_status(request):
    ...
    return render(
        request,
        "otheracademic/graduateSeminarStatus.html",
        {"form_data": form_data, "roll_no": roll_no},
    )
...
def graduateSeminarStatus_Dip(request):
    ...
    if obj:
        ...
        return render(
            request,
            "otheracademic/graduateSeminarStatus_Dip.html",
            {"form_data": form_data},
        )
    else:
        return HttpResponse("Not Available")
...
def bonafide_form_submit(request):
    ...
    if request.method == "POST":
        ...
        otheracademic_notif(
            request.user, bonafide_receiver, "bonafide", 1, "student", message
        )
...
def bonafideApproveForm(request):
    ...
    if "acadadmin" in design:
        ...
        return render(
            request, "otheracademic/bonafideApprove.html", {"form_data": form_data}
        )
    else:
        return HttpResponse("Not available For You")
...
def approve_bonafide(request, leave_id):
    ...
    otheracademic_notif(
        request.user, bonafide_aceptor, "bonafide_accept", 1, "student", message
    )
    return redirect(
        "/otheracademic/bonafideApproveForm"
    )  # Redirect to appropriate page after approval
...
def bonafideStatus(request):
    ...
    return render(
        request,
        "otheracademic/bonafideStatus.html",
        {"form_data": form_data, "roll_no": roll_no},
    )
...
def nodues_status(request):
    ...
    return render(
        request,
        "otheracademic/nodues_status.html",
        {"form_data": form_data, "roll_no": roll_no},
    )
...
def noduesStatus_acad(request):
    form_data = NoDues.objects.all()
    return render(
        request, "otheracademic/noduesStatus_acad.html", {"form_data": form_data}
    )
...
def nodues_apply(request):
    ...
    return render(
        request,
        "otheracademic/nodues_apply.html",
        {"form_data": form_data, "roll_no": roll_no},
    )
...
def update_dues_status(request):
    if request.method == "POST":
        ...
        try:
            ...
            student.save()
            return JsonResponse(
                {"message": "Clearance status updated successfully"}, status=200
            )
...
def submit_nodues_form(request):
    ...
    if request.method == "POST":
        ...
        otheracademic_notif(
            request.user, bank_receiver, "bank_nodues", nodues.id, "student", message
        )
        ...
        otheracademic_notif(
            request.user, btp_receiver, "btp_nodues", nodues.id, "student", message
        )
        ...
        otheracademic_notif(
            request.user, cse_receiver, "cse_nodues", nodues.id, "student", message
        )
        # design_receiver
        ...
        otheracademic_notif(
            request.user, acad_receiver, "acad_nodues", nodues.id, "student", message
        )
        ...
        otheracademic_notif(
            request.user, ece_receiver, "ece_nodues", nodues.id, "student", message
        )
        ...
        otheracademic_notif(
            request.user, me_receiver, "me_nodues", nodues.id, "student", message
        )
        ...
        otheracademic_notif(
            request.user, mess_receiver, "mess_nodues", nodues.id, "student", message
        )
        ...
        # discipline_receiver
        discipline_receiver = User.objects.get(
            username=data.get("discipline_credential")
        )
        file_bank = create_file(
            uploader=request.user.username,
            uploader_designation=obj,
...
def approve_BTP(request, no_dues_id):
    ...
    otheracademic_notif(
        request.user, btp_receiver, "nodues_approve", leave_entry.id, "student", message
    )
...
def Bank_nodues_not(request):
    ...
    form_data = NoDues.objects.filter(id__in=leave_ids)
    return render(
        request, "otheracademic/Bank_nodues_not.html", {"form_data": form_data}
    )
...
def BTP_nodues_not(request):
    ...
    form_data = NoDues.objects.filter(id__in=leave_ids)
    return render(
        request, "otheracademic/BTP_nodues_not.html", {"form_data": form_data}
    )
...
def CSE_nodues_not(request):
    ...
    form_data = NoDues.objects.filter(id__in=leave_ids)
    return render(
        request, "otheracademic/CSE_nodues_not.html", {"form_data": form_data}
    )
...
def Design_nodues_not(request):
    ...
    form_data = NoDues.objects.filter(id__in=leave_ids)
    return render(
        request, "otheracademic/Design_nodues_not.html", {"form_data": form_data}
    )
...
def dsa_nodues_not(request):
    ...
    form_data = NoDues.objects.filter(id__in=leave_ids)
    return render(
        request, "otheracademic/dsa_nodues_not.html", {"form_data": form_data}
    )
...
def Ece_nodues_not(request):
    ...
    form_data = NoDues.objects.filter(id__in=leave_ids)
    return render(
        request, "otheracademic/Ece_nodues_not.html", {"form_data": form_data}
    )
...
def hostel_nodues_not(request):
    ...
    form_data = NoDues.objects.filter(id__in=leave_ids)
    return render(
        request, "otheracademic/hostel_nodues_not.html", {"form_data": form_data}
    )
...
def mess_nodues_not(request):
    ...
    form_data = NoDues.objects.filter(id__in=leave_ids)
    return render(
        request, "otheracademic/mess_nodues_not.html", {"form_data": form_data}
    )
...
def Physics_nodues(request):
    ...
    form_data = NoDues.objects.filter(id__in=leave_ids)
    return render(
        request, "otheracademic/Physics_nodues.html", {"form_data": form_data}
    )
...
def Physics_nodues_not(request):
    ...
    form_data = NoDues.objects.filter(id__in=leave_ids)
    return render(
        request, "otheracademic/Physics_nodues_not.html", {"form_data": form_data}
    )
...
def discipline_nodues(request):
    ...
    form_data = NoDues.objects.filter(id__in=leave_ids)
    return render(
        request, "otheracademic/Discipline_nodues.html", {"form_data": form_data}
    )
...
def discipline_nodues_not(request):
    ...
    form_data = NoDues.objects.filter(id__in=leave_ids)
    return render(
        request, "otheracademic/Discipline_nodues_not.html", {"form_data": form_data}
    )
...
def library_nodues(request):
    ...
    form_data = NoDues.objects.filter(id__in=leave_ids)
    return render(
        request, "otheracademic/library_nodues.html", {"form_data": form_data}
    )
...
def library_nodues_not(request):
    ...
    form_data = NoDues.objects.filter(id__in=leave_ids)
    return render(
        request, "otheracademic/library_nodues_not.html", {"form_data": form_data}
    )
...
def assistantship_form_submission(request):
    ...
    if request.method == "POST":
        ...
        assistant = AssistantshipClaimFormStatusUpd.objects.create(
            student_name=request.user.first_name + request.user.last_name,
            roll_no=request.user.extrainfo,
        ...
        file_id = create_file(
            ...
            receiver=ta_supervisor,
            receiver_designation="student",
            src_module="otheracademic",
            src_object_id=assistant.id,
            file_extra_JSON={"value": 2},
            attached_file=None,
        ...
        otheracademic_notif(
            request.user, ta_supervisor_id, "ast_ta", assistant.id, "student", message
        )
        ...
        return HttpResponseRedirect(
            "/otheracademic/assistantship"
        )  # Replace '/otheracademic/assistantship' with the actual URL you want to redirect to
...
def assistantship_form_approval(request):
    ...
    assistantship_ids = [
        msg["src_object_id"] for msg in inbox if msg["subject"] == "assistantship"
    ]
    ...
    return render(
        request,
        "otheracademic/assistantship_approval.html",
        {"form_data": form_data, "roll_no": roll_no},
    )
...
def assistantship_thesis(request):
    ...
    return render(
        request,
        "otheracademic/thesis_supervisor_approve.html",
        {"form_data": form_data, "roll_no": roll_no},
    )
...
def assistantship_hod(request):
    ...
    return render(
        request,
        "otheracademic/hod_approval.html",
        {"form_data": form_data, "roll_no": roll_no},
    )
...
def assistantship_status(request):
    ...
    return render(
        request,
        "otheracademic/assistantship_status.html",
        {"form_data": form_data, "roll_no": roll_no},
    )
...
def assistantship_log(request):
    ...
    if user.extrainfo.department.name == "Academics":
        form_data = AssistantshipClaimFormStatusUpd.objects.all()
        return render(
            request, "otheracademic/assistantship_log.html", {"form_data": form_data}
        )
    else:
...
def assistanship_ta_approve(request, ass_id):
    ...
    otheracademic_notif(
        request.user, leave_receive, "ast_ta_accept", leave_entry.id, "student", message
    )
    ...
    forwarded_file_id = create_file(
        uploader=request.user.username,
        uploader_designation=des[0].designation,
        receiver=leave_entry.thesis_supervisor,
        receiver_designation="student",
        src_module="otheracademic",
        src_object_id=ass_id,
        file_extra_JSON={"value": 2},
        attached_file=None,
        subject="assistantship",
    )
    ...
    otheracademic_notif(
        request.user, thesis_supervisor_user, "ast_thesis", ass_id, "student", message
    )
...
def assistanship_thesis_approve(request, ass_id):
    ...
    otheracademic_notif(
        request.user, csehod_user, "ast_hod", ass_id, "student", message
    )
...
def assistanship_hod_approve(request, ass_id):
    ...
    forwarded_acad = create_file(
        uploader=request.user.username,
        uploader_designation=des[0].designation,
        receiver=acadadmin_receiver,
        receiver_designation="student",
        src_module="otheracademic",
        src_object_id=ass_id,
        file_extra_JSON={"value": 2},
    ...
    message = "Assistantship status received"
    otheracademic_notif(
        request.user, acadadmin_receiver, "ast_acadadmin", ass_id, "student", message
    )
...
def assistantship_acad_approveform(request):
    ...
    user = get_object_or_404(User, username=request.user.username)
    if user.extrainfo.department.name == "Academics":
        form_data = AssistantshipClaimFormStatusUpd.objects.filter(
            id__in=assistantship_ids
        )
        return render(
            request, "otheracademic/acadadmin_approval.html", {"form_data": form_data}
        )
    else:
        return HttpResponse("Not Avalable For You.")
