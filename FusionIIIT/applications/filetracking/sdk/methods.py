...
def view_inbox(username: str, designation: str, src_module: str) -> list:
    ...
    received_files_serialized = list(
        FileHeaderSerializer(received_files_unique, many=True).data
    )
...
def view_drafts(username: str, designation: str, src_module: str) -> dict:
    ...
    draft_files = File.objects.filter(
        ...
        uploader=user_ExtraInfo_object,
...
def view_history(file_id: int) -> dict:
    ...
    """
    Tracking_history = Tracking.objects.filter(file_id=file_id).order_by(
        "-receive_date"
    )
...
def get_designations(username: str) -> list:
    ...
    designation_name = [
        hold_designation.designation.name for hold_designation in designations_held
    ]
...
def add_uploader_department_to_files_list(files: list) -> list:
    ...
    for file in files:
        ...
        if uploader_Extrainfo.department is None:
            ...
            file["uploader_department"] = (str(uploader_Extrainfo.department)).split(
                ": "
            )[1]
...
def get_HoldsDesignation_obj(username: str, designation: str) -> HoldsDesignation:
    ...
    user_designation = get_designation_obj_from_name(designation=designation)
    obj = HoldsDesignation.objects.get(user=user_object, designation=user_designation)
    return obj


def get_last_recv_tracking_for_user(
    file_id: int, username: str, designation: str
) -> Tracking:
    """
    ...
    last_tracking = Tracking.objects.filter(
        file_id=file_id, receiver_id=recv_user_obj, receive_design=recv_design_obj
    ).order_by("-receive_date")[0]
    return last_tracking


def get_last_forw_tracking_for_user(
    file_id: int, username: str, designation: str
) -> Tracking:
    """
    ...
    sender_designation_obj = get_HoldsDesignation_obj(
    ...
    last_tracking = (
        Tracking.objects.filter(
            ...
            current_design=sender_designation_obj,
        )
