...
def filetracking(request):
    ...
    designation_name = request.session.get(
    ...
    designation_id = get_HoldsDesignation_obj(username, designation_name).id
...
def draft_design(request):
    ...
    username = request.user
    dropdown_HoldsDesignation_obj = get_HoldsDesignation_obj(username, dropdown_design)
...
def outbox_view(request):
    ...
    for f in outward_files:
        last_forw_tracking = get_last_forw_tracking_for_user(
            file_id=f["id"],
            username=user_HoldsDesignation_obj.user,
            designation=user_HoldsDesignation_obj.designation,
        )
    ...
    if subject_query:
        outward_files = [
            f for f in outward_files if subject_query.lower() in f["subject"].lower()
        ]
    if sent_to_query:
        outward_files = [
            f
            for f in outward_files
            if sent_to_query.lower() in f["sent_to_user"].username.lower()
        ]
    if date_query:
        try:
            ...
            outward_files = [
                f
                for f in outward_files
                if f["last_sent_date"].date() == search_date.date()
            ]
        except ValueError:
...
def inbox_view(request):
    ...
    for f in inward_files:
        ...
        f["is_forwarded"] = (
            False
            if (
                str(get_current_file_owner(f["id"]).username)
                == str(user_HoldsDesignation_obj.user)
            )
            else True
        )
    ...
    if subject_query:
        inward_files = [
            f for f in inward_files if subject_query.lower() in f["subject"].lower()
        ]
    if sent_to_query:
        inward_files = [
            f
...
def view_file(request, id):
    ...
    if (
        ...
        and last_receiver_designation == file.designation.name
        and file_uploader == request.user
        and file.is_read is False
    ):
        archive_enable = True
...
def unarchive_file(request, id):
    try:
        ...
        messages.error(request, f"Unable to unarchive: {str(e)}")
...
def download_file(request, id):
    ...
    for t in track:
        ...
        elements.append(Spacer(1, 12))
        remarks = (
            f"<b>Remarks:</b> {t.remarks}"
            if t.remarks
            else "<b>Remarks:</b> No Remarks"
        )
        elements.append(Paragraph(remarks, style_paragraph))
        elements.append(Spacer(1, 12))
        attachment = (
            f"<b>Attachment:</b> {os.path.basename(t.upload_file.name)}"
            if t.upload_file
        ...
        elements.append(
            Paragraph(
                '<hr width="100%" style="border-top: 1px solid #ccc;">', style_paragraph
            )
        )
