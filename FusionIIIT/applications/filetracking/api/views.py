...
class ViewOutboxView(APIView):
    ...
    def get(self, request):
        ...
        if not username or not src_module:
            return Response(
                {
                    "error": "Missing required query parameters: username and src_module."
                },
                status=400,
            )
    ...
    def get(self, request, file_id):
        ...
        try:
            ...
            for history in histories:
                ...
                temp_obj_action["receiver_id"] = User.objects.get(
                    id=history["receiver_id"]
                ).username
                temp_obj_action["receive_design"] = Designation.objects.get(
                    id=history["receive_design"]
                ).name
    ...
    def post(self, request, file_id):
        ...
        if not receiver or not receiver_designation:
            raise ValidationError(
                "Missing required fields: receiver and receiver_designation"
            )
        ...
        try:
            ...
            logging.info(
                f"Successfully forwarded file {file_id} with tracking ID: {new_tracking_id}"
            )
        except Exception as e:
            logging.error(f"Error forwarding file {file_id}: {str(e)}")
            raise ValidationError(
                str(e)
            )  # Re-raise exception with a user-friendly message

        # Return response
        return Response(
            {"tracking_id": new_tracking_id}, status=status.HTTP_201_CREATED
        )
