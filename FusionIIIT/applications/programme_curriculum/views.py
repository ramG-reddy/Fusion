...
from .filters import BatchFilter, CourseFilter, CurriculumFilter
from .forms import (
    BatchForm,
    CourseForm,
    CourseProposalTrackingFile,
    CourseSlotForm,
    CurriculumForm,
    DisciplineForm,
...
def programme_curriculum(request):
    ...
    if (
        request.session["currentDesignationSelected"] == "student"
        or request.session["currentDesignationSelected"] == "Associate Professor"
        or request.session["currentDesignationSelected"] == "Professor"
        or request.session["currentDesignationSelected"] == "Assistant Professor"
        or request.session["currentDesignationSelected"] == "Dean Academic"
    ):
...
def view_all_programmes(request):
    ...
    if request.session["currentDesignationSelected"] == "acadadmin":
        ...
        request.session["currentDesignationSelected"] == "Associate Professor"
        or request.session["currentDesignationSelected"] == "Professor"
        or request.session["currentDesignationSelected"] == "Assistant Professor"
        or request.session["currentDesignationSelected"] == "Dean Academic"
    ):
...
def view_all_working_curriculums(request):
    ...
    return render(
        request,
        url + "view_all_working_curriculums.html",
        {
...
def view_semesters_of_a_curriculum(request, curriculum_id):
    ...
    return render(
        request,
        url + "view_semesters_of_a_curriculum.html",
        {
...
def view_a_semester_of_a_curriculum(request, semester_id):
    ...
    return render(
        request,
        url + "view_a_semester_of_a_curriculum.html",
        {
            "semester": semester,
...
def add_course_form(request):
    ...
    return render(
        request,
        "programme_curriculum/acad_admin/course_form.html",
        {"form": form, "submitbutton": submitbutton},
    )
            ...
            if form.is_valid():
                ...
                if new_course.version > previous.version:
                    ...
                    if old_course:
                        ...
                        for i in old_course:
                            if set(form.cleaned_data["disciplines"]) != set(
                                i.disciplines.all()
                            ) or set(form.cleaned_data["pre_requisit_courses"]) != set(
                                i.pre_requisit_courses.all()
                            ):
                                add = True
                            else:
                                add = False
                        ...
                        if add:
                            ...
                            new_course.pre_requisit_courses.set(
                                form.cleaned_data["pre_requisit_courses"]
                            )
                            course = Course.objects.last()
                            messages.success(request, "Added successful")
                            return HttpResponseRedirect(
                                "/programme_curriculum/admin_course/"
                                + str(course.id)
                                + "/"
                            )
                        else:
                            form.add_error(
                                None,
                                'A Course with the same values already exists at "Version Number '
                                + " "
                                + str(ver)
                                + '"',
                            )
                    ...
                    version_error += (
                        f"The version should be greater than {previous.version}"
                    )
...
def view_course_proposal_forms(request):
    ...
    notifs = request.user.notifications.all()
    courseProposal = NewProposalFile.objects.filter(
        uploader=des.user,
        designation=request.session["currentDesignationSelected"],
        is_update=False,
    )
    updatecourseProposal = NewProposalFile.objects.filter(
        uploader=des.user,
        designation=request.session["currentDesignationSelected"],
        is_update=True,
    )
    ...
    return render(
        request,
        "programme_curriculum/faculty/view_course_proposal_forms.html",
        {
            "courseProposals": courseProposal,
            "updateProposals": updatecourseProposal,
            "data": data,
            "notifications": notifs,
        },
    )
...
def faculty_view_all_courses(request):
    ...
    return render(
        request,
        "programme_curriculum/faculty/faculty_view_all_courses.html",
        {
            "courses": courses,
            "coursefilter": coursefilter,
            "notifications": notifs,
...
def faculty_view_a_course(request, course_id):
    ...
    return render(
        request,
        "programme_curriculum/faculty/faculty_view_a_course.html",
        {
            "course": course,
            "notifications": notifs,
        },
...
def view_a_course_proposal_form(request, CourseProposal_id):
    ...
    return render(
        request,
        "programme_curriculum/faculty/view_a_course_proposal.html",
        {
...
def new_course_proposal_file(request):
    ...
    return render(
        request,
        "programme_curriculum/faculty/course_proposal_form.html",
        {"form": form, "submitbutton": submitbutton},
    )
...
def filetracking(request, proposal_id):
    ...
    form = CourseProposalTrackingFile(
        initial={
            "current_id": file.uploader,
            "current_design": file.designation,
            "file_id": int(proposal_id),
        }
    )
        ...
        if request.method == "POST":
            ...
            if form.is_valid():
                try:
                    ...
                    form.add_error(
                        None,
                        "Proposal_ tracking with this File id, Current id, Current design and Disciplines already exists.",
                    )

    return render(
        request,
        "programme_curriculum/faculty/filetracking.html",
        {
            "form": form,
            "submitbutton": submitbutton,
            "file_info": file_data,
...
def inward_files(request):
    ...
    designation = Designation.objects.get(
        name=request.session["currentDesignationSelected"]
    )
    des_id = designation.id

    courseProposal = Proposal_Tracking.objects.filter(
        receive_design=des_id, receive_id=id
    )

    return render(
        request,
        "programme_curriculum/faculty/inward_course_forms.html",
        {
            "courseProposals": courseProposal,
            "design": request.session["currentDesignationSelected"],
            "data": data,
...
def outward_files(request):
    ...
    courseProposal = Proposal_Tracking.objects.filter(
        current_design=design, current_id=des.user
    )

    return render(
        request,
        "programme_curriculum/faculty/outward_course_forms.html",
        {
            "courseProposals": courseProposal,
            "design": request.session["currentDesignationSelected"],
            "data": data,
...
def update_course_proposal_file(request, course_id):
    ...
    form = NewCourseProposalFile(
        initial={"uploader": des.user, "designation": design}, instance=course
    )
    submitbutton = request.POST.get("Submit")
        ...
        if request.method == "POST":
            ...
            if form.is_valid():
                ...
                if old_course:
                    # Check if disciplines or pre_requisit_courses have been changed
                    for i in old_course:
                        if set(form.cleaned_data["pre_requisit_courses"]) != set(
                            i.pre_requisit_courses.all()
                        ):
                            add = True
                        else:
...
def forward_course_forms(request, ProposalId):
    ...
    if request.session["currentDesignationSelected"] == "Dean Academic":
        ...
        if course:
            previous = (
                Course.objects.all().filter(code=course.code).order_by("version").last()
            )
            course.version = previous.version
                ...
                if request.method == "POST":
                    ...
                    if form.is_valid():
                        ...
                        if new_course.version > previous.version:
                            ...
                            return HttpResponseRedirect(
                                "/programme_curriculum/course/" + str(course.id) + "/"
                            )
                        else:
                            version_error += (
                                f"The version should be greater than {previous.version}"
            ...
            return render(
                request,
                "programme_curriculum/faculty/dean_view_a_course_proposal.html",
                {
                    "course": course,
                    "form": form,
                    "submitbutton": submitbutton,
        ...
        form = CourseProposalTrackingFile(
            initial={
                "current_id": des.user,
                "current_design": request.session["currentDesignationSelected"],
            ...
            if request.method == "POST":
                ...
                if form.is_valid():
                    try:
                        ...
                        prog_and_curr_notif(request.user, receiver_id, data)
                        return HttpResponseRedirect(
                            "/programme_curriculum/outward_files/"
                        )
                    except IntegrityError:
                        form.add_error(
                            None,
    ...
    return render(
        request,
        "programme_curriculum/faculty/forward.html",
        {
            "form": form,
...
def view_inward_files(request, ProposalId):
    ...
    courseProposal = Proposal_Tracking.objects.filter(
        file_id=file.file_id, disciplines=file.disciplines
    )
    form = CourseProposalTrackingFile(
        initial={
            "current_id": des.user,
    ...
    return render(
        request,
        "programme_curriculum/faculty/view_file.html",
        {
            "form": form,
            "receive_date": file.receive_date,
            "proposal": file2,
            "trackings": courseProposal,
            "file_info": file_data,
            "file_sucess": file_data2,
        },
    )
...
def reject_form(request, ProposalId):
    ...
    if not track.is_added and not track.is_submitted:
        ...
        data = (
            ...
            + str(request.user)
            + " - "
            + str(request.session["currentDesignationSelected"])
        )
        prog_and_curr_notif(request.user, receiver_id, data)
...
def tracking_unarchive(request, ProposalId):
    ...
    print(request.user)
    if str(track.current_design) == str(
        request.session["currentDesignationSelected"]
    ) and str(track.current_id) == str(request.user):
