...
from .forms import *
from .models import *
from .models import financial_outlay, projects
...
def add_projects(request):
    ...
    if request.method == "POST":
        ...
        check = HoldsDesignation.objects.filter(
            user__username=pid, designation__name="Professor"
        )  # checking for pid to exist

        if not check.exists():
            check = HoldsDesignation.objects.filter(
                user__username=pid, designation__name="Assistant Professor"
            )

            if not check.exists():
                messages.error(
                    request, "Request not added, no such project investigator exists "
                )
                return render(request, "rs/projects.html")

        check = HoldsDesignation.objects.filter(
            user__username=copid, designation__name="Professor"
        )  # checking for copid to exist

        if not check.exists():
            check = HoldsDesignation.objects.filter(
                user__username=copid, designation__name="Assistant Professor"
            )

            if not check.exists():
                messages.error(
                    request,
                    "Request not added, no such co project investigator exists ",
                )
                return render(request, "rs/projects.html")
        ...
        for key, value in obj.items():
            if key.startswith("co_project_investigator_id-"):
                if value not in copi_list:
                    check = HoldsDesignation.objects.filter(
                        user__username=value, designation__name="Professor"
                    )  # checking for copid to exist
                    if not check.exists():
                        check = HoldsDesignation.objects.filter(
                            user__username=value,
                            designation__name="Assistant Professor",
                        )
                        if not check.exists():
                            messages.error(
                                request,
        ...
        for project in obj:
            if project.project_name == projectname:
                messages.error(
                    request, "Request not added, project name already exists"
                )
                return render(request, "rs/projects.html")
        ...
        for copi in copi_list:
            if copi == pid:
                messages.error(
                    request,
                    "Request not added, project investigator and co project investigator cannot be same",
                )
                return render(request, "rs/projects.html")
        ...
        pi_designation = HoldsDesignation.objects.get(
            user=userpi_instance, designation__name="Professor"
        )
        if not pi_designation:
            pi_designation = HoldsDesignation.objects.get(
                user=userpi_instance, designation__name="Assistant Professor"
            )
...
def add_requests(request, id, pj_id):
    if request.method == "POST":
        ...
        if id == "0":
            ...
            check = projects.objects.filter(
                project_id=projectid, project_investigator_id__username=pi_id
            )
            if not check.exists():
                messages.error(
                    request, "Request not added, no such project investigator exists"
...
def view_projects(request):
    ...
    queryset = projects.objects.filter(
        project_investigator_id__username=request.user.username
    )
    start_index = (int(page_number) - 1) * projects_per_page if page_number else 0
    end_index = start_index + projects_per_page
...
def view_requests(request, id):
    if id == "1":
        ...
        queryset = rspc_inventory.objects.filter(
            project_investigator_id=request.user.username
        )
    else:
...
def view_financial_outlay(request, pid):
    table_data = financial_outlay.objects.filter(project_id=pid).order_by(
...
def submit_closure_report(request, id):
    ...
    data = {"projects": queryset, "username": request.user.username}
...
def view_project_inventory(request, pj_id):
    ...
    data = {"requests": queryset, "username": request.user.username}
...
def financial_outlay_form(request, pid):
    ...
    categories_with_subcategories = category.objects.values(
...
def add_staff_details(request, pid):
        ...
        for key, value in obj.items():
            if key.startswith("staff_id"):
                ...
                if not User.objects.filter(username=staff_id).exists():
                    messages.error(
                        request, "Staff with ID " + staff_id + " does not exist"
                    )
                    return redirect(
                        "/research_procedures/add_staff_details/" + str(pid)
                    )
    ...
    years_passed = int(
        (datetime.datetime.now().date() - project.start_date).days / 365.25
    )
...
def view_staff_details(request, pid):
    ...
    # Pass the organized data to the template
    context = {"data_by_year": data_by_year, "project_name": project.project_name}
    rspc_admin = HoldsDesignation.objects.get(designation__name="rspc_admin")
...
def inbox(request):
    ...
    data = Tracking.objects.filter(
        receiver_id=user_obj,
        receive_design=user_designation,
        file_id__src_module="research_procedures",
...
def view_file(request, id):
    ...
    current_user = (
        Tracking.objects.filter(file_id=file1).order_by("-receive_date")[0].current_id
    )

    return render(
        request,
        "rs/view_file.html",
        context={"file": file1, "tracks": tracks, "current_user": current_user},
    )
...
def forward_request(request, id):
    ...
    if request.method == "POST":
        ...
        if receiver_designation == "project_investigator":
            ...
            receiver_instance = HoldsDesignation.objects.get(
                designation__name=receiver_designation
            ).user
        ...
        if (
            receiver_designation == "Professor"
            or receiver_designation == "Assistant Professor"
            or receiver_designation == "rspc_admin"
        ):
...
def update_financial_outlay(request, pid):
    ...
    if request.method == "POST":
        ...
        financial_outlay_instance = financial_outlay.objects.get(
            financial_outlay_id=financial_outlay_id
        )
...
def download_project_pdf(request, project_id):
    ...
    financial_outlay_data = [
        ["Financial Outlay ID", "Category", "Subcategory", "Amount", "Year"]
    ]
    for outlay in financial_outlays:
        financial_outlay_data.append(
            [
                outlay.financial_outlay_id,
                outlay.category,
                outlay.sub_category,
                outlay.amount,
                outlay.year,
    ...
    financial_outlay_table.setStyle(
        TableStyle(
            [
                ("BACKGROUND", (0, 0), (-1, 0), colors.grey),
                ("TEXTCOLOR", (0, 0), (-1, 0), colors.whitesmoke),
...
def change_end_date(request, id):
    if request.method == "POST":
        ...
        staff_allocation_instance = staff_allocations.objects.get(
            staff_allocation_id=staff_allocations_id
        )
    ...
    return redirect(
        "/research_procedures/view_staff_details/"
        + str(staff_allocation_instance.project_id.project_id)
    )
...
def get_obj_by_username_and_designation(username, designation):
    ...
    return HoldsDesignation.objects.filter(
        user=user_instance, designation=designation_instance
    )
