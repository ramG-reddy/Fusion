...
def update_last_selected_role(request):
    ...
    return Response(
        {"message": "last_selected_role updated successfully"},
...
def NotificationUnread(request):
    try:
        ...
        if not notification.unread:
            ...
            notification.save()
        response = {"message": "Notification successfully marked as unread."}
        return Response(response, status=status.HTTP_200_OK)
    except:
        response = {"error": "Failed to mark the notification as unread."}
        return Response(response, status=status.HTTP_404_NOT_FOUND)
...
def delete_notification(request):
    try:
        ...
        response = {"message": "Notification marked as deleted."}
        return Response(response, status=status.HTTP_200_OK)
