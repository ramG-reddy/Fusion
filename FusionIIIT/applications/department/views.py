...
from .models import Announcements, Information, SpecialRequest
...
def all_students(request, bid):
    ...
    if sort_by:
        ...
        try:
            student_list1 = (
                Student.objects.order_by(sort_by)
                .filter(id__user_type="student", **filter_criteria)
                .select_related("id")
            )
        except:
            # If the sort field doesn't exist or isn't sortable, ignore the sort parameter
            student_list1 = Student.objects.filter(
                id__user_type="student", **filter_criteria
            ).select_related("id")
        request.session["last_sort"] = (
            sort_by  # Save the sort parameter for the next request
        )
    else:
        student_list1 = Student.objects.filter(
            id__user_type="student", **filter_criteria
...
def alumni(request):
    ...
    context_a = {"cse_a": cse_a, "ece_a": ece_a, "me_a": me_a, "sm_a": sm_a}
