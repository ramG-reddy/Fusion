...
class StockEntry(models.Model):
    ...
    location = models.CharField(
        max_length=100, choices=Constants.Locations, default="SR1"
    )
...
class StockItem(models.Model):
    ...
    )  # Unique identifier for each StockItem
    ...
    def save(self, *args, **kwargs):
        ...
        if not self.nomenclature:
            ...
            max_existing_number = StockItem.objects.filter(
                StockEntryId=self.StockEntryId_id
            ).count()
...
class StockTransfer(models.Model):
    ...
    dest_location = models.CharField(
        max_length=100, choices=Constants.Locations, default="SR2"
    )
    ...
    if created:
        ...
        for _ in range(current_stock):
            StockItem.objects.create(
                ...
                department=instance.item_id.file_info.uploader.department,
            )
