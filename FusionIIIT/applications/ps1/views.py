...
def ps1(request):
    ...
    isdept_admin = any(
        designation in dept_admin_to_dept for designation in designations
    )

    if isdept_admin or currentDes == "ps_admin":
        return redirect("/purchase-and-store/entry/")
    elif "student" not in designations and str(request.user.extrainfo.department.name):
    ...
    if request.method == "POST":
        try:
            if "save" in request.POST:
                ...
                design = HoldsDesignation.objects.filter(
                    user=request.user,
                    designation__name=request.session["currentDesignationSelected"],
                ).first()
...
def archieveview(request, id):
    ...
    archived_files = view_archived(
        username=request.user, designation=designations, src_module="ps1"
    )
...
def indentview2(request, id):
    ...
    data = sorted(
        data, key=lambda x: datetime.fromisoformat(x["upload_date"]), reverse=True
    )
    ...
    context = {"receive_design": abcd, "in_file": data, "notifications": notifs}
    ...
    if request.method == "POST":
        ...
        if "Archieve" in request.POST:
            ...
            if is_archived:
                ...
                return redirect(f"/purchase-and-store/forwardedIndent/{file.id}")
            else:
        ...
        if "send" in request.POST:
            ...
            sender_designationobj = HoldsDesignation.objects.get(
                id=sender_design_id
            ).designation
            ...
            ) and (str(receive_design) in ["Director", "Registrar"]):
                indent.head_approval = True
            elif (sender_designation_name in ["Director", "Registrar"]) and (
                str(receive_design) in dept_admin_design
            ):
                indent.director_approval = True
            elif (sender_designation_name == "Accounts Admin") and (
                (str(receive_design) in dept_admin_design)
                or str(receive_design) == "ps_admin"
            ):
...
def delete(request, id):
    if request.session["currentDesignationSelected"] not in dept_admin_design + [
        "ps_admin"
    ]:
        return redirect("/dashboard")
    file = File.objects.get(pk=id)
    ...
    if request.method == "POST":
        ...
        return render(
            ...
            "ps1/stock_edit.html",
            {
                ...
                "persons": persons,
                "formatted_received_date": formatted_received_date,
            },
        )
...
def stock_update(request):
    ...
    if request.method == "POST":
        if "save" in request.POST:
            ...
            dealing_assistant_id_str = (
                request.POST.get("dealing_assistant_id").split("-")[0].strip()
            )
            print({dealing_assistant_id_str})

            dealing_assistant_instance = ExtraInfo.objects.get(
                id=dealing_assistant_id_str
            )
...
def current_stock_view(request):
    ...
    if request.method == "POST":
        ...
        StockItems = StockItem.objects.filter(
            department=department, StockEntryId__item_id__item_type=type
        )

        grouped_items = StockItems.values(
            "StockEntryId__item_id__item_type",
            "StockEntryId__item_id__item_subtype",
            "department",
    ...
    if request.session["currentDesignationSelected"] in dept_admin_design:
        ...
        deptId = DepartmentInfo.objects.values("id", "name").get(
            name=dept_admin_to_dept[request.session["currentDesignationSelected"]]
        )
    ...
    grouped_items_list = [
        {
            "item_type": item["StockEntryId__item_id__item_type"],
            "item_subtype": item["StockEntryId__item_id__item_subtype"],
            "department": DepartmentInfo.objects.values("id", "name").get(
                id=item["department"]
            ),
    ...
    return render(
        request,
        "ps1/current_stock_view_filter.html",
        {
...
def stock_entry_view(request):
    ...
    if request.session["currentDesignationSelected"] in dept_admin_design:
        ...
        deptId = DepartmentInfo.objects.get(
            name=dept_admin_to_dept[request.session["currentDesignationSelected"]]
        )
        stocks = StockEntry.objects.filter(
            item_id__file_info__uploader__department=deptId
        )
    elif request.session["currentDesignationSelected"] == "ps_admin":
...
def stock_entry_item_view(request):
    ...
    return render(
        request,
        "ps1/stock_entry_item_view.html",
        {"stocks": stocks, "first_stock": stocks.first()},
    )
...
def entry(request):
    ...
    if request.method == "POST":
        ...
        return render(
            ...
            {
                "id": id,
                "indent": CorrespondingIndentFile,
    ...
    if request.session["currentDesignationSelected"] in dept_admin_design:
        IndentFiles = IndentFile.objects.filter(
            file_info__uploader__department__name=dept_admin_to_dept[
                request.session["currentDesignationSelected"]
            ]
        )
    else:
...
def generate_report(request):
    ...
    if request.method == "GET":
        ...
        return render(
            request, "ps1/generate_report_filter.html", {"departments": departments}
        )
    ...
    if request.method == "POST":
        ...
        if departments[0] == "all":
            StockItems = StockItem.objects.filter(
                StockEntryId__recieved_date__range=(start_date, finish_date)
            )
        else:
            StockItems = StockItem.objects.filter(
        ...
        grouped_items_list = [
            {
                ...
                "StockItems": StockItem.objects.filter(
                    ...
                    StockEntryId__item_id__item_type=item[
                        "StockEntryId__item_id__item_type"
                    ],
                ),
            }
        ...
        return render(
            request,
            "ps1/generate_report.html",
            {"departments": departments, "stocks": grouped_items_list},
        )
...
def view_bill(request, stock_entry_id):
    ...
    if stock_entry.bill:
        ...
        response["Content-Disposition"] = (
            f'attachment; filename="{stock_entry.bill.name}"'
        )
        return response
    else:
...
def stock_transfer(request):
    ...
    if request.method == "POST":
        ...
        return render(
            request,
            "ps1/stock_transfer.html",
            {"indentFile": temp1, "available_items": available_items},
        )
...
def item_detail(request, id):
    ...
    context = {"transfers": stock_transfers, "item": stock_item}
