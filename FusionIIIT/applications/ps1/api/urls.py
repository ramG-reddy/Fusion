...
urlpatterns = [
    ...
    url(
        r"^indentFile/forward/(?P<id>\d+)$",
        views.ForwardIndentFile,
        name="forward-indent-file",
    ),
    # STOCKS API
    ...
    url(
        r"^stock_entry_view/(?P<id>\d+)$", views.stockEntryView, name="stock-entry-view"
    ),
    # To view the current_stock_view based on filters
    url(
        r"^current_stock_view/(?P<id>\d+)$",
        views.currentStockView,
        name="current-stock-view",
    ),
    # to view all the items present in any stock entry
    url(
        r"^stock_entry_item_view/(?P<id>\d+)$",
        views.stock_entry_item_view,
        name="stock-entry-item-view",
    ),
    ...
    url(
        r"^perform_transfer/(?P<id>\d+)$",
        views.performTransfer,
