...
def draftView(request, id):
    if request.method == "GET":
        ...
        draft_indent = IndentFile.objects.filter(file_info__in=draft).values(
            "file_info"
        )
...
def inwardIndents(request, id):
    if request.method == "GET":
        ...
        response_data = {"receive_design": str(abcd), "in_file": data}
...
def indentFile(request, id):
    if request.method == "GET":
        try:
            indent_file = IndentFile.objects.select_related("file_info").get(
                file_info=id
            )
        except IndentFile.DoesNotExist:
            return Response(
                {"message": "Indent file does not exist"},
        ...
        track = Tracking.objects.select_related(
            ...
            "receiver_id",
            "receive_design",
        ).filter(file_id=indent_file.file_info)
        ...
        holdsdesignations_serializer = HoldsDesignationSerializer(
            holdsdesignations, many=True
        )
...
def ForwardIndentFile(request, id):
    if request.method == "POST":
        ...
        if str(receive_design) in dept_admin_design:
            ...
            or (sender_designation_name == "ps_admin")
    ...
    if request.method == "GET":
        ...
        if request.session.get("currentDesignationSelected") == "dept_admin":
            indent_files = IndentFile.objects.filter(
                file_info__uploader__department__name=department
            )
        else:
        ...
        try:
            ...
            serializer = {
                ...
                "corresponding_indent_file": IndentFileSerializer(
                    corresponding_indent_file
                ).data,
            }
            ...
            return Response(
                {"message": "Corresponding indent file does not exist"},
                status=status.HTTP_404_NOT_FOUND,
            )
...
def stockEntryView(request, id):
    ...
    if designation in dept_admin_design:
        stocks = StockEntry.objects.filter(
            item_id__file_info__uploader__department=department
        )
    elif designation == "ps_admin":
        stocks = StockEntry.objects.all()
    else:
        return Response(
            {"message": "Invalid designation"}, status=status.HTTP_400_BAD_REQUEST
        )
...
def currentStockView(request, id):
    ...
    if request.method == "GET":
        ...
        if designation in dept_admin_design:
            ...
            return Response(
                {"message": "Invalid designation"}, status=status.HTTP_403_FORBIDDEN
            )

        grouped_items = stocks.values(
            "StockEntryId__item_id__item_type", "department"
        ...
        StockItems = StockItem.objects.filter(
            department=department, StockEntryId__item_id__item_type=item_type
        )
...
def stockDelete(request, id):
    ...
    try:
        ...
        return Response(
            {"message": "Stock item with given ID does not exist", "id": id},
            status=status.HTTP_404_NOT_FOUND,
        )

    stock.delete()
    return Response(
        {"message": "Stock item deleted successfully"}, status=status.HTTP_200_OK
    )
...
def stockTransfer(request, id):
    ...
    available_items = StockItem.objects.filter(
        StockEntryId__item_id__item_type=item_type_required, inUse=False
    )
