...
def compounder_view_handler(request):
    ...
    # edit Threshold
        ...
        try:
            ...
            if Required_medicine.objects.filter(medicine_id=threshold_med).exists():
                ...
                medicine_stock = Present_Stock.objects.filter(
                    Q(medicine_id=threshold_med) & Q(Expiry_date__gt=date.today())
                )
                qty = 0
    ...
    # edit schedule for doctors
        ...
        schedule = Doctors_Schedule.objects.select_related("doctor_id").filter(
        ...
        if schedule.count() == 0:
            ...
            Doctors_Schedule.objects.select_related("doctor_id").filter(
    ...
    # edit schedule for pathologists
        ...
        schedule = Pathologist_Schedule.objects.select_related("pathologist_id").filter(
            pathologist_id=doctor, day=day
        )
        if schedule.count() == 0:
            Pathologist_Schedule.objects.create(
            ...
            Pathologist_Schedule.objects.select_related("pathologist_id").filter(
                pathologist_id=pathologist_id, day=day
            ).update(room=room)
            Pathologist_Schedule.objects.select_related("pathologist_id").filter(
                pathologist_id=pathologist_id, day=day
            ).update(from_time=time_in)
    ...
    #     return HttpResponse(sches, content_type='json')
        ...
        try:
            ...
            if id == 0:
                status = 1
                similar_name_qs = All_Medicine.objects.filter(
                    brand_name__istartswith=medicine_name
                )[:10]
            else:
                status = 2
                similar_name_qs = All_Medicine.objects.filter(id=id)
            similar_name = list(
                similar_name_qs.values(
                    "id",
                    "medicine_name",
                    "constituents",
                    "manufacturer_name",
            ...
            try:
                ...
                stk = Stock_entry.objects.filter(medicine_id=med).order_by(
                    "Expiry_date"
                )
                for s in stk:
            ...
            return JsonResponse(
                {"val": val_to_return, "sim": similar_name, "status": status}
            )
    ...
    #     return HttpResponse(sches, content_type='json')
        ...
        if uploaded_file != None:
            ...
            new_file = files.objects.create(file_data=f)
            fid = new_file.id
        ...
        for med in medicine:
            ...
            if stock == "," or stock == "N/A at moment,":
                ...
                p_stock.save()
                stock_of_medicine = Present_Stock.objects.filter(
                    Q(medicine_id=med_id) & Q(Expiry_date__gt=date.today())
                )
                qty = 0
                ...
                if qty < med_id.threshold:
                    if Required_medicine.objects.filter(medicine_id=med_id).exists():
                        ...
                        Required_medicine.objects.create(
                            medicine_id=med_id, quantiy=qty, threshold=med_id.threshold
                        )
            ...
            with transaction.atomic():
                for _, row in df.iterrows():
                    med = All_Medicine.objects.get(
                        Q(brand_name=row["brand_name"])
                        & Q(manufacturer_name=row["manufacturer_name"])
                        & Q(pack_size_label=row["packsize"])
                    )
                    stk = Stock_entry.objects.create(
                        id=tot_rows + t,
        ...
        user_ids = HoldsDesignation.objects.filter(
            designation_id=acc_admin_des_id.id
        ...
        forwarded_file_id = forward_file(
            ...
            file_extra_JSON={"value": 2},
            remarks="Forwarded File with id: "
            + str(request.POST["file_id"])
            + "to Accounts Admin "
            + str(acc_admins.id),
            file_attachment=None,
        )

        medical_relief_instance = medical_relief.objects.get(
            file_id=request.POST["file_id"]
        )
        ...
        new_report = []
        new_prescriptions = All_Prescription.objects.filter(
            Q(user_id__icontains=search) | Q(details__icontains=search)
        ).order_by("-date", "-id")[new_offset : new_offset + page_size]
        total_count = All_Prescription.objects.filter(
            Q(user_id__icontains=search) | Q(details__icontains=search)
        ).count()
        total_pages = (total_count + page_size - 1) // page_size
        for pre in new_prescriptions:
            doc = None
            if pre.doctor_id != None:
                doc = pre.doctor_id.doctor_name
            dic = {
                "id": pre.pk,
                "user_id": pre.user_id,
                "date": pre.date,
        ...
        return JsonResponse(
            {
                ...
                "previous_page_number": new_current_page - 1
                if new_current_page > 1
                else None,
                "next_page_number": new_current_page + 1
        ...
        new_live_meds = []
        new_live = Stock_entry.objects.filter(
            Q(Expiry_date__gte=date.today())
            & Q(
                Q(medicine_id__brand_name__icontains=search)
                | Q(supplier__icontains=search)
            )
        ).order_by("Expiry_date")[new_offset_stock : new_offset_stock + page_size_stock]
        total_pages_stock = (
            Stock_entry.objects.filter(
                Q(Expiry_date__gte=date.today())
                & Q(
                    Q(medicine_id__brand_name__icontains=search)
                    | Q(supplier__icontains=search)
        ...
        return JsonResponse(
            {
                ...
                "previous_page_number": new_current_page_stock - 1
                if new_current_page_stock > 1
                else None,
                "next_page_number": new_current_page_stock + 1
                if new_current_page_stock < total_pages_stock
                else None,
            }
        )
    elif (
        "datatype" in request.POST
        and request.POST["datatype"] == "manage_stock_expired"
    ):
        ...
        new_offset_stock_expired = (
            new_current_page_stock_expired - 1
        ) * new_page_size_stock_expired
        new_expired = []
        new_expiredData = Stock_entry.objects.filter(
            Q(Expiry_date__lt=date.today())
            & Q(
                Q(medicine_id__brand_name__icontains=search)
                | Q(supplier__icontains=search)
            )
        ).order_by("Expiry_date")[
            new_offset_stock_expired : new_offset_stock_expired
            + new_page_size_stock_expired
        ]
        ...
        return JsonResponse(
            {
                ...
                "has_next": new_current_page_stock_expired
                < new_total_pages_stock_expired,
                "previous_page_number": new_current_page_stock_expired - 1
                if new_current_page_stock_expired > 1
                else None,
                "next_page_number": new_current_page_stock_expired + 1
                if new_current_page_stock_expired < new_total_pages_stock_expired
                else None,
            }
        )
    elif (
        "datatype" in request.POST
        and request.POST["datatype"] == "manage_stock_required"
    ):
        ...
        new_offset_stock_required = (
            new_current_page_stock_required - 1
        ) * new_page_size_stock_required
        new_required = []
        new_requiredData = Required_medicine.objects.filter(
            Q(medicine_id__brand_name__icontains=search)
        )[
            new_offset_stock_required : new_offset_stock_required
            + new_page_size_stock_required
        ]
        new_total_pages_stock_required = (
            Required_medicine.objects.filter(
                Q(medicine_id__brand_name__icontains=search)
            ).count()
            + new_page_size_stock_required
            - 1
        ...
        return JsonResponse(
            {
                ...
                "has_next": new_current_page_stock_required
                < new_total_pages_stock_required,
                "previous_page_number": new_current_page_stock_required - 1
                if new_current_page_stock_required > 1
                else None,
                "next_page_number": new_current_page_stock_required + 1
                if new_current_page_stock_required < new_total_pages_stock_required
                else None,
            }
        )
        ...
        prescriptions = All_Prescription.objects.filter(
            Q(user_id__icontains=search) | Q(details__icontains=search)
        ).order_by("-date", "-id")[offset : offset + page_size_prescription]
        ...
        total_pages = (
            total_count + page_size_prescription - 1
        ) // page_size_prescription  # This ensures rounding up
        prescContext = {
            "count": total_pages,
            "page": {
                "object_list": report,
        ...
        live_meds = []
        live = Stock_entry.objects.filter(
            Q(Expiry_date__gte=date.today())
            & Q(
                Q(medicine_id__brand_name__icontains=search)
                | Q(supplier__icontains=search)
            )
        ).order_by("Expiry_date")[offset_stock : offset_stock + page_size_stock]
        total_pages_stock = (
            Stock_entry.objects.filter(
                Q(Expiry_date__gte=date.today())
                & Q(
        ...
        stockContext = {
            ...
            "page_stock_view": {
                ...
                "previous_page_number": current_page_stock - 1
                if current_page_stock > 1
                else None,
                "next_page_number": current_page_stock + 1
                if current_page_stock < total_pages_stock
                else None,
            },
        }
        ...
        offset_stock_expired = (
            current_page_stock_expired - 1
        ) * page_size_stock_expired
        expired = []
        expiredData = Stock_entry.objects.filter(
            Q(Expiry_date__lt=date.today())
            & Q(
                Q(medicine_id__brand_name__icontains=search)
                | Q(supplier__icontains=search)
            ...
            offset_stock_expired : offset_stock_expired + page_size_stock_expired
        ]
        total_pages_stock_expired = (
            Stock_entry.objects.filter(
                Q(Expiry_date__lt=date.today())
                & Q(
                    Q(medicine_id__brand_name__icontains=search)
        ...
        ExpiredstockContext = {
            ...
            "page_stock_expired": {
                ...
                "previous_page_number": current_page_stock_expired - 1
                if current_page_stock_expired > 1
                else None,
                "next_page_number": current_page_stock_expired + 1
                if current_page_stock_expired < total_pages_stock_expired
                else None,
            },
        }
        ...
        required_data = Required_medicine.objects.filter(
            Q(medicine_id__brand_name__icontains=search)
        )[offset_stock_required : offset_stock_required + page_size_required]
        total_pages_stock_required = (
            Required_medicine.objects.filter(
                Q(medicine_id__brand_name__icontains=search)
            ).count()
            + page_size_required
        ...
        stocks = {
            ...
            "page_stock_required": {
                ...
                "previous_page_number": current_required_page - 1
                if current_required_page > 1
                else None,
                "next_page_number": current_required_page + 1
                if current_required_page < total_pages_stock_required
                else None,
            },
        }
...
def student_view_handler(request):
    if "amb_submit" in request.POST:
        ...
        doctor_id = request.POST.get("doctor")
        days = (
            Dotors_Schedule.objects.select_related("doctor_id")
            .filter(doctor_id=doctor_id)
            .values("day")
        )
        today = datetime.today()
        time = datetime.today().time()
        sch = Doctors_Schedule.objects.select_related("doctor_id").filter(
            date__gte=today
        )
        ...
        form_object = medical_relief(description=description, file=uploaded_file)
        ...
        Announcements.objects.select_related(
            "user_id",
            "user_id__user",
            "user_id__department",
            "message",
            "upload_announcement",
        ).filter(pk=anno_id).delete()
        data = {"status": 1}
        ...
        MedicalProfile.objects.select_related(
            "user_id",
            "user_id__user",
            "user_id__department",
            "date_of_birth",
