...
from .models import (
    ...
    Present_Stock,
    Required_medicine,
    Required_tabel_last_updated,
    Stock_entry,
    files,
    medical_relief,
)
from .utils import compounder_view_handler, student_view_handler
...
def compounder_view(request):
    ...
    if design == "Compounder":
        if request.method == "POST":
            ...
            schedule1 = (
                Pathologist_Schedule.objects.select_related("pathologist_id")
                .all()
                .order_by("pathologist_id")
            )
            ...
            if fir == None:
                ...
                for e in exp:
                    med = e.medicine_id
                    p_s = Present_Stock.objects.filter(
                        Q(medicine_id=med) & Q(Expiry_date__gte=date.today())
                    )
                    qty = 0
                    for ps in p_s:
                        qty += ps.quantity
                    if Required_medicine.objects.filter(medicine_id=med).exists():
                        ...
                        if qty >= med.threshold:
                            ...
                            req.quantity = qty
                            req.save()
                    elif qty < med.threshold:
                        Required_medicine.objects.create(
                            medicine_id=med, quantity=qty, threshold=med.threshold
                        )
                Required_tabel_last_updated.objects.create(date=date.today())
            else:
                last_updated = fir.date
                exp = Stock_entry.objects.filter(
                    Q(Expiry_date__gte=last_updated) & Q(Expiry_date__lt=date.today())
                )
                for e in exp:
                    med = e.medicine_id
                    p_s = Present_Stock.objects.filter(
            ...
            total_pages_stock_required = (
                Required_medicine.objects.all().count() + page_size_required - 1
            ) // page_size_required
            stocks = {
            ...
            total_pages_stock_expired = (
                Stock_entry.objects.filter(Expiry_date__lt=date.today()).count()
                + page_size_stock_expired
                - 1
            ) // page_size_stock_expired
            ...
            live_meds = []
            live = Stock_entry.objects.filter(Expiry_date__gte=date.today()).order_by(
                "Expiry_date"
            )[offset_stock : offset_stock + page_size_stock]
            total_pages_stock = (
                Stock_entry.objects.filter(Expiry_date__gte=date.today()).count()
                + page_size_stock
                - 1
            ) // page_size_stock
            for e in live:
            ...
            prescriptions = All_Prescription.objects.all().order_by("-date", "-id")[
                offset : offset + page_size_prescription
            ]
            ...
            for ib in inbox_files:
                ...
                for mr in medicalrelief:
                    if mr.file_id == int(ib["id"]):
                        ...
                        dic["upload_date"] = datetime.fromisoformat(
                            ib["upload_date"]
                        ).date()
            ...
            return render(
                ...
                "phcModule/phc_compounder.html",
                {
                    "days": days,
                    "users": users,
                    "expired": ExpiredstockContext,
                    "stocks": stocks,
                    "doctors": doctors,
                    "pathologists": pathologists,
                    "schedule": schedule,
                    "schedule1": schedule1,
                    "live_meds": stockContext,
                    "presc_hist": prescContext,
                    "inbox_files": inbox,
                    "medicines_presc": medicine_presc,
                },
            )
    else:
        ...
        if request.method == "POST":
            ...
            for out in uploader_outbox:
                ...
                for inb in uploader_inbox:
                    if dic["id"] == inb["id"]:
                        dic["approval_date"] = datetime.fromisoformat(
                            inb["upload_date"]
                        ).date()
...
def pathologist_entry(request):
    ...
    """
    excel = xlrd.open_workbook(
        os.path.join(os.getcwd(), "dbinsertscripts/healthcenter/Pathologist-List.xlsx")
    )
    z = excel.sheet_by_index(0)
    ...
    for i in range(1, 5):
        try:
            ...
            u = Pathologist.objects.create(
                pathologist_name=name, pathologist_phone=phone, specialization=spl
            )
...
def announcement(request):
    ...
    return render(
        request,
        "health_center/make_announce_comp.html",
        {
...
def medical_profile(request):
    ...
    if request.method == "POST":
        user_info = (
            ExtraInfo.objects.all().select_related("user", "department").get(id=user_id)
        )
    ...
    return render(
        request,
        "health_center/medical_profile.html",
        {
            "user_designation": user_info.user_type,
...
def compounder_view_prescription(request, prescription_id):
    ...
    doctors = Doctor.objects.filter(active=True).order_by("id")
    follow_presc = Prescription_followup.objects.filter(
    ...
    return render(
        request,
        "phcModule/phc_compounder_view_prescription.html",
        {
            "prescription": prescription,
            "pre_medicine": pre_medicine,
            "doctors": doctors,
...
def view_file(request, file_id):
    ...
    if file_id_int == -2:
        return FileResponse(
            open("static/health_center/add_stock_example.xlsx", "rb"),
            as_attachment=True,
            filename="example_add_stock.xlsx",
        )
    if file_id_int == -1:
        return FileResponse(
            open("static/health_center/add_medicine_example.xlsx", "rb"),
            as_attachment=True,
            filename="example_add_medicine.xlsx",
        )
    ...
    with open(
        "applications/health_center/static/health_center/generated.pdf", "wb+"
    ) as destination:
        destination.write(f)
