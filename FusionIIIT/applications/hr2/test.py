def ltc_pre_processing(request):
    ...
    ltc_form_data["leave_availability"] = request.POST.getlist(
        "leave_availability"
    ) == ["True", "True"]
    ...
    for i in range(1, 7):
        if request.POST.get(f"info_{i}_1"):
            family_member = ",".join(
                request.POST.getlist(f"info_{i}_{j}")[0] for j in range(1, 4)
            )
    ...
    for i in range(1, 7):
        if request.POST.get(f"d_info_{i}_1"):
            dependent = ",".join(
                request.POST.getlist(f"d_info_{i}_{j}")[0] for j in range(1, 5)
            )
            dependents.append(dependent)
    ltc_form_data["details_of_dependents"] = ",".join(dependents)

    # Extract remaining fields
    ltc_form_data["amount_of_advance_required"] = int(
        request.POST["amount_of_advance_required"]
    )
    ltc_form_data["certified_family_dependents"] = request.POST[
    ...
    ltc_form_data["phone_number_for_contact"] = int(
        request.POST["phone_number_for_contact"]
    )
...
request_data = {
    "csrfmiddlewaretoken": [
        "yLyPMZMWRBnDU3hSh5kPGq6AgOFNY5WTK1HaZxAuiozCzXBf8qfOML5irZJd8MkM"
    ],
    "block_year": ["232"],
    "pf_no": ["222"],
