    ...
    def post(self, request):
        ...
        if employee.user_type == "faculty":
            ...
            if request.method == "POST":
                ...
                detailsOfFamilyMembers = ", ".join(
                    filter(None, [family_mem_a, family_mem_b, family_mem_c])
                )

                request.POST = request.POST.copy()
                request.POST["detailsOfFamilyMembersAlreadyDone"] = (
                ...
                for i in range(
                    1, 7
                ):  # Loop through input fields for each family member
                ...
                for i in range(1, 7):  # Loop through input fields for each dependent
                    ...
                    why_dependent = request.POST.get(
                        f"d_info_{i}_4", ""
                    )  # Get the reason for dependency
                    if name and age:  # Check if both name and age are provided
                        dependents.append(
                            f"{name} ({age} years), {why_dependent}"
                        )  # Concatenate name, age, and reason
                ...
                pf_no = (
                    int(request.POST.get("pf_no"))
                    if request.POST.get("pf_no")
                    else None
                )
                basicPay = (
                    int(request.POST.get("basicPay"))
                    if request.POST.get("basicPay")
                    else None
                )
                amountOfAdvanceRequired = (
                    int(request.POST.get("amountOfAdvanceRequired"))
                    if request.POST.get("amountOfAdvanceRequired")
                    else None
                )
                phoneNumberForContact = (
                    int(request.POST.get("phoneNumberForContact"))
                    if request.POST.get("phoneNumberForContact")
                    else None
                )
                ...
                try:
                    ltc_request = LTCform.objects.create(
                        employee_id=id,
                        detailsOfFamilyMembersAlreadyDone=request.POST.get(
                            "detailsOfFamilyMembersAlreadyDone", ""
                        ),
                        familyMembersAboutToAvail=request.POST.get(
                            "familyMembersAboutToAvail", ""
                        ),
                        ...
                        dateOfLeaveForFamily=request.POST.get(
                        ...
                        detailsOfFamilyMembers=request.POST.get(
                            "detailsOfFamilyMembers", ""
                        ),
                        amountOfAdvanceRequired=request.POST.get(
                            "amountOfAdvanceRequired", ""
                        ),
                        certifiedFamilyDependents=request.POST.get(
                        ...
                        phoneNumberForContact=request.POST.get(
                            "phoneNumberForContact", ""
                        ),
                    )
                    print("done")
