...
def ltc_pre_processing(request):
    ...
    for memeber in request.POST.getlist("detailsOfFamilyMembersAlreadyDone"):
        if memeber == "":
            detailsOfFamilyMembersAlreadyDone = (
                detailsOfFamilyMembersAlreadyDone + "None" + ","
            )
        else:
            detailsOfFamilyMembersAlreadyDone = (
                detailsOfFamilyMembersAlreadyDone + memeber + ","
            )

    data["detailsOfFamilyMembersAlreadyDone"] = (
        detailsOfFamilyMembersAlreadyDone.rstrip(",")
    )
    ...
    for i in range(1, 4):
        for j in range(1, 4):
            ...
            if request.POST.get(key_is) == "":
                detailsOfFamilyMembersAboutToAvail = (
                    detailsOfFamilyMembersAboutToAvail + "None" + ","
                )
            else:
                detailsOfFamilyMembersAboutToAvail = (
                    detailsOfFamilyMembersAboutToAvail + request.POST.get(key_is) + ","
                )

    data["detailsOfFamilyMembersAboutToAvail"] = (
        detailsOfFamilyMembersAboutToAvail.rstrip(",")
    )
    ...
    for i in range(1, 7):
        for j in range(1, 5):
            ...
            if request.POST.get(key_is) == "":
                ...
                detailsOfDependents = (
                    detailsOfDependents + request.POST.get(key_is) + ","
                )
...
def reverse_ltc_pre_processing(data):
    ...
    reversed_data["detailsOfFamilyMembersAlreadyDone"] = getattr(
        data, "detailsOfFamilyMembersAlreadyDone"
    ).split(",")

    detailsOfFamilyMembersAboutToAvail = getattr(
        data, "detailsOfFamilyMembersAboutToAvail"
    ).split(",")
...
def ltc_form(request, id):
    ...
    if (
        employee.user_type == "faculty"
        or employee.user_type == "staff"
        or employee.user_type == "student"
    ):
        ...
        if request.method == "POST":
            try:
                ...
                form1 = {
                    ...
                    "dateOfDepartureForFamily": request.POST.get(
                        "dateOfDepartureForFamily"
                    ),
                    ...
                    "detailsOfFamilyMembersAlreadyDone": data[
                        "detailsOfFamilyMembersAlreadyDone"
                    ],
                    "detailsOfFamilyMembersAboutToAvail": data[
                        "detailsOfFamilyMembersAboutToAvail"
                    ],
                    ...
                    "certifiedThatFamilyDependents": request.POST.get(
                        "certifiedThatFamilyDependents"
                    ),
                    "certifiedThatAdvanceTakenOn": request.POST.get(
                        "certifiedThatAdvanceTakenOn"
                    ),
        ...
        inbox = view_inbox(
            username=username, designation=uploader_designation, src_module="HR"
        )
        ...
        context = {
            ...
            "archived_files": filtered_archived_files,
            "user_id": user_id,
        }
...
def get_current_file_owner(file_id: int) -> User:
    ...
    """
    latest_tracking = (
        Tracking.objects.filter(file_id=file_id).order_by("-receive_date").first()
    )
...
def file_handle_leave(request):
    if request.method == "POST":
        ...
        try:
            ...
            return JsonResponse(
                {"error": "LeaveForm object with the provided ID does not exist"},
                status=404,
            )
        ...
        if action == "0":
            if remark == "":
                track_id = forward_file(
                    file_id=file_id,
                    receiver=username_employee,
                    receiver_designation=designation_employee,
                    remarks=f"Forwarded by {current_owner} to {username_employee}",
                ...
                track_id = forward_file(
                    ...
                    receiver=username_employee,
                    receiver_designation=designation_employee,
                    remarks=f"Forwarded by {current_owner} to {username_employee}, Reason : {remark}",
                    file_extra_JSON="None",
                )
            ...
            if remark == "":
                ...
                track_id = forward_file(
                    ...
                    receiver=leave_form.name,
                    receiver_designation=leave_form.designation,
                    remarks=f"Rejected by {current_owner}, Reason : {remark}",
                    file_extra_JSON="None",
                )
            ...
            if remark == "":
                ...
                track_id = forward_file(
                    ...
                    receiver=leave_form.name,
                    receiver_designation=leave_form.designation,
                    remarks=f"Approved by {current_owner}, Reason : {remark}",
                    file_extra_JSON="None",
                )
...
def file_handle_cpda(request):
    if request.method == "POST":
        ...
        try:
            ...
            return JsonResponse(
                {"error": "CPDAform object with the provided ID does not exist"},
                status=404,
            )
        ...
        if action == "0":
            ...
            if remark == "":
                track_id = forward_file(
                    file_id=file_id,
                    receiver=cpda_form.name,
                    receiver_designation=cpda_form.designation,
                    remarks=f"Rejected by {current_owner}",
                    file_extra_JSON="None",
                )
            else:
...
def file_handle_cpda_reimbursement(request):
    if request.method == "POST":
        ...
        try:
            ...
            return JsonResponse(
                {
                    "error": "CPDAReimbursementform object with the provided ID does not exist"
                },
                status=404,
            )
...
def file_handle_ltc(request):
    if request.method == "POST":
        ...
        try:
            ...
            return JsonResponse(
                {"error": "LTCform object with the provided ID does not exist"},
                status=404,
            )
...
def file_handle_appraisal(request):
    if request.method == "POST":
        ...
        try:
            ...
            return JsonResponse(
                {"error": "Appraisalform object with the provided ID does not exist"},
                status=404,
            )
        ...
        if action == "0":
            ...
            if remark == "":
                track_id = forward_file(
                    file_id=file_id,
                    receiver=appraisal_form.name,
                    receiver_designation=appraisal_form.designation,
                    remarks=f"Rejected by {current_owner}",
                    file_extra_JSON="None",
                )
            else:
...
def view_ltc_form(request, id):
    ...
    context = {"ltc_request": ltc_request}
...
def view_cpda_form(request, id):
    ...
    context = {"cpda_request": cpda_request}
        ...
        if request.method == "POST":
            try:
                form_3 = {
                    ...
                    "purposeOfLeave": request.POST.get("purposeOfLeave"),
                    "addressDuringLeave": request.POST.get("addressDuringLeave"),
                    "academicResponsibility": request.POST.get(
                        "academicResponsibility"
                    ),
                    "addministrativeResponsibiltyAssigned": request.POST.get(
                        "addministrativeResponsibiltyAssigned"
                    ),
                    "created_by": creator,
                }
        ...
        if request.method == "POST":
            try:
                ...
                form_4 = {
                    ...
                    "specificFieldOfKnowledge": request.POST.get(
                        "specificFieldOfKnowledge"
                    ),
                    "currentResearchInterests": request.POST.get(
                    ...
                    "otherInstructionalTasks": request.POST.get(
                    ...
                    "expertLectureDelivered": request.POST.get(
                    ...
                    "administrativeAssignment": request.POST.get(
...
def view_appraisal_form(request, id):
    ...
    context = {"appraisal_request": appraisal_request}
    ...
    for i in range(3, 5):
        for j in range(1, 4):
            ...
            if request.POST.get(key_is) == "":
                ...
                newCoursesIntroduced = (
                    newCoursesIntroduced + request.POST.get(key_is) + ","
                )
    ...
    for i in range(5, 7):
        for j in range(1, 5):
            ...
            if request.POST.get(key_is) == "":
                ...
                newCoursesDeveloped = (
                    newCoursesDeveloped + request.POST.get(key_is) + ","
                )
    ...
    for i in range(9, 10):
        for j in range(1, 8):
            ...
            if request.POST.get(key_is) == "":
                ...
                sponsoredReseachProjects = (
                    sponsoredReseachProjects + request.POST.get(key_is) + ","
                )
...
def cpda_reimbursement_form(request, id):
    ...
    if (
        ...
        cpda_reimbursement_requests = CPDAReimbursementform.objects.filter(
            employeeId=id
        )
...
def form_view_cpda_reimbursement(request, id):
    ...
    cpda_reimbursement_request = reverse_cpda_reimbursement_pre_processing(
        cpda_reimbursement_request
    )
...
def view_cpda_reimbursement_form(request, id):
    ...
    context = {"cpda_reimbursement_request": cpda_reimbursement_request}
...
def form_mangement_cpda_reimbursement(request):
    if request.method == "GET":
        ...
        for src_object_id in src_object_ids:
            cpda_reimbursement_request = get_object_or_404(
                CPDAReimbursementform, id=src_object_id
            )
                ...
                form_data.append(
                    {
                        ...
                        "submissionDate": form.submissionDate.strftime("%Y-%m-%d")
...
def getformcpdaReimbursement(request):
    ...
    if form_type == "CPDAReimbursement":
        try:
            ...
            return JsonResponse(
                {"message": "No CPDAReimbursement forms found."}, status=404
            )
