...
def healthcare_center_notif(sender, recipient, type, message):
    ...
    notify.send(
        sender=sender, recipient=recipient, url=url, module=module, verb=verb, flag=flag
    )
...
def office_module_DeanS_notif(sender, recipient, type):
    ...
    if type == "hostel_alloted":
        ...
        verb = (
            "Insufficient Funds! Please contact Junior Superintendent to allot funds."
        )
...
def AssistantshipClaim_notify(sender, recipient, month, year):
    message = f"Your Assistantshipclaim of {month} month year {year} is approved "
    url = "academic-procedures:academic_procedures"
    module = "Assistantship Request"
    notify.send(
        sender=sender, recipient=recipient, url=url, module=module, verb=message
    )
...
def AssistantshipClaim_account_notify(sender, stu, recipient):
    message = f"Assistantship claim of{stu} is forwaded "
    url = "academic-procedures:academic_procedures"
    module = "Assistantship Request"
    notify.send(
        sender=sender, recipient=recipient, url=url, module=module, verb=message
...
def course_management_notif(
    sender, recipient, type, course, course_name, course_code, flag
):
